...
type BaseSelectVariant = 'default' | 'outlined';
...
const VARIANT_CLASSES: Record<BaseSelectVariant, string> = {
    default:
        'w-full p-2 text-sm transition-shadow duration-300 border-none rounded-xs outline-none bg-tertiary shadow-default focus:shadow-inside hover:shadow-inside',
    outlined:
...
const BaseSelect = ({
    ...
    const triggerClass = className
        ...
        : `${VARIANT_CLASSES[variant]} disabled:opacity-50 disabled:cursor-not-allowed ${error ? 'ring-1 ring-red-500' : ''}`;
