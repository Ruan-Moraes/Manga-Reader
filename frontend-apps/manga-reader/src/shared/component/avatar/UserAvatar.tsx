...
const avatarColors = ['bg-indigo-600', 'bg-emerald-600', 'bg-amber-600', 'bg-rose-600', 'bg-cyan-600', 'bg-violet-600', 'bg-teal-600', 'bg-fuchsia-600'];
...
const UserAvatar = ({ src, name, size = 'md', rounded = 'xs', className }: UserAvatarProps) => {
    ...
    return (
        ...
        >
            {showImage ? (
                <img src={src} alt={`Foto de ${name}`} className="object-cover w-full h-full" onError={() => setImgError(true)} />
            ) : (
                <div className={clsx('flex items-center justify-center w-full h-full font-bold text-white', config.text)}>{letter}</div>
            )}
        </div>
    );
};
