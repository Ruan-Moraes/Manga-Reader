...
import { useCommentPagination, CommentUser } from '@feature/comment';
...
const CATEGORY_META: {
    ...
    iconType: IllustrationType;
    i18nKey: string;
}[] = [
    { key: 'funRating', iconType: 'feliz', i18nKey: 'fun' },
    { key: 'artRating', iconType: 'pensando', i18nKey: 'art' },
    { key: 'storylineRating', iconType: 'feliz', i18nKey: 'storyline' },
    { key: 'charactersRating', iconType: 'feliz', i18nKey: 'characters' },
    { key: 'originalityRating', iconType: 'surpresa', i18nKey: 'originality' },
    { key: 'pacingRating', iconType: 'duvida', i18nKey: 'pacing' },
];
...
const RecentReviews = ({ ratings }: RecentReviewsProps) => {
    ...
    const { visibleItems, hasMore, loadMore } = useCommentPagination(ratings, REVIEWS_PER_PAGE);
        ...
        return (
            <section className="flex flex-col gap-2 p-4 border rounded-xs border-tertiary">
                ...
                <div className="flex items-center justify-center p-4 border border-dashed border-tertiary rounded-xs">
                    <p className="text-xs text-center text-tertiary">{t('recent.empty')}</p>
                </div>
            </section>
        );
    }
    ...
    return (
        <section className="flex flex-col gap-2 p-2 rounded-xs bg-secondary border border-tertiary">
            ...
            {visibleItems.map(review => (
                <article key={review.id} className="flex flex-col gap-2 p-2 border rounded-xs border-tertiary bg-primary-default">
                    <CommentUser size="sm" user={{ name: review.userName }} createdAt={review.createdAt} onClickProfile={handleClickProfile} />
                    <div className="grid grid-cols-2 pt-1 gap-x-4 gap-y-2">
                        {CATEGORY_META.map(meta => {
                            const value = review[meta.key] as number;

                            return (
                                <div key={meta.key} className="flex items-center justify-between gap-1">
                                    <span className="flex items-center gap-1 text-[0.65rem]">
                                        <Illustration type={meta.iconType} className="w-4 h-4 object-contain" />
                                        <span className="text-tertiary">{t(`categories.${meta.i18nKey}`)}</span>
                                    </span>
                    ...
                    {review.comment && <p className="pt-1 text-xs text-tertiary">{review.comment}</p>}
                </article>
            ))}
            {hasMore && (
                <Button variant="ghost" size="sm" onClick={loadMore} block>
                    {t('recent.loadMore')}
                </Button>
            )}
        </section>
