...
import { type EventType, type EventData } from '../model/event.types';
...
export const getEvents = async (page = 0, size = 20): Promise<PageResponse<EventData>> => {
    const response = await api.get<ApiResponse<PageResponse<EventData>>>(API_URLS.EVENTS, { params: { page, size } });

    return response.data.data;
};

export const getEventById = async (id: string): Promise<EventData> => {
    const response = await api.get<ApiResponse<EventData>>(`${API_URLS.EVENTS}/${id}`);

    return response.data.data;
};

export const getRelatedEvents = async (eventId: string): Promise<EventData[]> => {
    const response = await api.get<ApiResponse<EventData[]>>(`${API_URLS.EVENTS}/${eventId}/related`);

    return response.data.data;
};

export const eventTypes: EventType[] = ['Convenção', 'Lançamento', 'Live', 'Workshop', 'Meetup'];
...
export const filterEvents = (items: EventData[], filters: EventFilters): EventData[] => {
