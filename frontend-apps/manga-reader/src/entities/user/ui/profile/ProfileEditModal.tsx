...
import { type EnrichedProfile, type SocialLinkResponse } from '../../model/user.types';
...
type Props = {
    ...
    isOpen: boolean;
    onClose: () => void;
    onSave: (data: { name: string; bio: string; photoUrl: string; bannerUrl: string; socialLinks: { platform: string; url: string }[] }) => Promise<void>;
};
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
            <div className="w-full max-w-lg max-h-[90vh] overflow-y-auto bg-primary-default border border-tertiary rounded-xs p-4">
                <div className="flex items-center justify-between mb-4">
                    <h2 className="text-lg font-bold">{t('profile.edit.title')}</h2>
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-3">
                    ...
                    <div>
                        <div className="flex items-center justify-between mb-2">
                            <span className="text-sm font-medium">{t('profile.edit.socialLinksLabel')}</span>
                            <button type="button" onClick={handleAddLink} className="text-xs text-quaternary hover:underline">
                        ...
                        {socialLinks.map((link, i) => (
                            <div key={link.id} className="flex gap-2 mb-2">
                                <input
                                    ...
                                    placeholder={t('profile.edit.platformPlaceholder')}
                                ...
                                />
                                <button type="button" onClick={() => handleRemoveLink(i)} className="text-xs text-quinary-default">
                                    X
                                </button>
                            </div>
                        ))}
