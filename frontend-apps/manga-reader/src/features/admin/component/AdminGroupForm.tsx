...
const AdminGroupForm = () => {
    ...
    return (
        <div className="flex flex-col gap-4">
            ...
            <h1 className="text-lg font-bold">{t('dashboard.groups.editTitle', 'Editar Grupo')}</h1>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-3">
                ...
                <label className="flex flex-col gap-1">
                    <span className="text-sm text-tertiary">{t('groupForm.logo', 'Logo (URL)')}</span>
                    <input value={logo} onChange={e => setLogo(e.target.value)} className="px-3 py-2 text-sm border rounded-xs bg-secondary border-tertiary" />
                </label>

                <label className="flex flex-col gap-1">
                    <span className="text-sm text-tertiary">{t('groupForm.banner', 'Banner (URL)')}</span>
                    <input
                        value={banner}
                ...
                <label className="flex flex-col gap-1">
                    <span className="text-sm text-tertiary">{t('groupForm.website', 'Website')}</span>
                    <input
                        value={website}
