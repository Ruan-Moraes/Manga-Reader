...
const groupNameById = new Map(mockGroups.map(group => [group.id, group.name]));

mockGroups.forEach(group => {
    group.members.forEach(member => {
        if (!member.groups.find(memberGroup => memberGroup.id === group.id)) {
...
export const getAllGenres = () =>
    Array.from(new Set(mockGroups.flatMap(group => group.genres))).sort();

export const getGroupStatusLabel = (status: GroupStatus) => {
...
export const getMemberById = (memberId: string) => {
    const member = membersPool.find(item => item.id === memberId);

    if (!member) return null;

    return {
        ...member,
        groups: member.groups.filter(group => groupNameById.has(group.id)),
    };
};
