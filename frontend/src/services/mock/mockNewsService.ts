...
export const filterNews = ({
    ...
    const limitDate =
        period === 'all'
            ? null
            : new Date(Date.now() - periodInDays[period] * 24 * 60 * 60 * 1000);
    ...
    const filtered = mockNews.filter(news => {
        ...
        if (source !== 'all' && news.source !== source) {
            return false;
        }

        if (limitDate && new Date(news.publishedAt) < limitDate) {
            return false;
        }
...
export const getRelatedNews = (news: NewsItem, limit = 6) =>
    mockNews
...
export const getNewsSources = () =>
    ['all', ...Array.from(new Set(mockNews.map(news => news.source)))] as const;

export const isNewsFresh = (publishedAt: string) =>
