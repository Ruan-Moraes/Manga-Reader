    ...
    return (
        <button
            ...
            className={`flex items-center gap-2 px-3 py-2 text-xs font-semibold border rounded-xs transition-all ${
                isSaved ? 'border-ui-accent-border bg-quaternary-default/25' : 'border-tertiary bg-secondary hover:bg-primary-default'
            }`}
        >
            {isLoading ? <span>...</span> : isSaved ? <Bookmark className="text-ui-accent-fg animate-pulse" /> : <Bookmark />}
