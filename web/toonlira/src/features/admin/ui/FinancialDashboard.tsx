    ...
    return (
        <div className="flex flex-col gap-4">
            ...
            <div className="rounded-ui-md border border-ui-border bg-ui-surface p-[18px]">
                <h3 className="mb-3 text-[15px] font-ui-bold text-ui-fg">{t('dashboard.financial.summary.distributionByStatus')}</h3>
                <div className="overflow-x-auto">
                    <table className="w-full text-ui-small">
                        <thead>
                            <tr className="border-b border-ui-border">
                                <th className="py-2 text-left text-ui-tiny font-ui-extrabold uppercase tracking-[0.1em] text-ui-fg-subtle">{t('dashboard.financial.columnStatus')}</th>
                                <th className="py-2 text-right text-ui-tiny font-ui-extrabold uppercase tracking-[0.1em] text-ui-fg-subtle">{t('dashboard.financial.summary.quantity')}</th>
                                <th className="py-2 text-right text-ui-tiny font-ui-extrabold uppercase tracking-[0.1em] text-ui-fg-subtle">{t('dashboard.financial.summary.totalAmount')}</th>
                            </tr>
                        </thead>
                        <tbody>
                            {statuses.map(status => (
                                <tr key={status} className="border-b border-ui-gray-900 last:border-b-0">
                                    <td className="py-2.5">
                                        <StatusPill tone={toneFor(PAYMENT_STATUS_TONE, status)}>{t(statusLabelKey('payment', status), { defaultValue: status })}</StatusPill>
                                    </td>
                                    <td className="py-2.5 text-right tabular-nums text-ui-fg">{summary.countsByStatus[status] ?? 0}</td>
                                    <td className="py-2.5 text-right tabular-nums text-ui-fg-subtle">{formatCurrency(summary.amountsByStatus[status] ?? 0)}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
