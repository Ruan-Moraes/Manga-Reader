...
const buildColumns = (t: TFunction, lang: LanguageTag, onEdit: (event: AdminEvent) => void, onDelete: (event: AdminEvent) => void): Column<AdminEvent>[] => [
    {
        ...
        hideBelow: 'md',
        render: event => <span className="font-ui-mono text-ui-tiny text-ui-fg-subtle">{event.id.slice(0, 8)}</span>,
    },
    {
        key: 'title',
        header: t('dashboard.events.columnTitle'),
        sortable: true,
        render: event => <span className="font-ui-bold text-ui-fg">{event.title?.[lang] ?? event.title?.['pt-BR'] ?? ''}</span>,
    },
    {
        key: 'type',
    ...
    },
    {
        ...
        hideBelow: 'md',
        render: event => <span className="text-ui-fg-subtle">{event.locationIsOnline ? t('dashboard.events.locationOnline') : (event.locationCity ?? '—')}</span>,
    },
    {
        key: 'startDate',
        ...
        sortable: true,
        hideBelow: 'sm',
        render: event => <span className="text-ui-fg-subtle">{formatDate(event.startDate)}</span>,
    },
    {
        key: 'actions',
