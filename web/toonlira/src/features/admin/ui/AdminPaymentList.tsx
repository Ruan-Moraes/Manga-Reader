...
const buildColumns = (t: TFunction, onEdit: (payment: AdminPayment) => void, onDelete: (payment: AdminPayment) => void): Column<AdminPayment>[] => [
    {
        ...
        hideBelow: 'md',
        render: payment => <span className="font-ui-mono text-ui-tiny text-ui-fg-subtle">{payment.id.slice(0, 8)}</span>,
    },
    {
        key: 'amount',
        header: t('dashboard.financial.columnAmount'),
        sortable: true,
        render: payment => <span className="font-ui-bold tabular-nums text-ui-fg">{formatAmount(payment.amount, payment.currency)}</span>,
    },
    {
        key: 'status',
    ...
    },
    {
        ...
        hideBelow: 'md',
        sortable: true,
        render: payment => <span className="font-ui-mono text-ui-tiny text-ui-fg-subtle">{payment.paymentMethod ?? '—'}</span>,
    },
    {
        key: 'referenceType',
        header: t('dashboard.financial.columnReference'),
        hideBelow: 'md',
        render: payment => <span className="font-ui-mono text-ui-tiny text-ui-fg-subtle">{payment.referenceType ?? '—'}</span>,
    },
    {
        key: 'createdAt',
        ...
        sortable: true,
        hideBelow: 'sm',
        render: payment => <span className="text-ui-fg-subtle">{formatDate(payment.createdAt)}</span>,
    },
    {
        key: 'actions',
