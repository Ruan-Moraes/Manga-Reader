...
const CustomTooltip = ({ active, payload }: { active?: boolean; payload?: Array<{ payload: MonthlyRevenueEntry }> }) => {
    ...
    return (
        <div className="rounded-ui-xs border border-ui-border bg-ui-surface p-3 text-ui-small shadow-ui-black">
            <p className="font-ui-bold text-ui-fg">{formatMonth(data.yearMonth, i18n.language)}</p>
            <p className="text-ui-accent-fg">
                {t('revenueChart.revenue', {
                    value: formatCurrency(data.revenue, i18n.language),
                })}
            </p>
            <p className="text-ui-fg-subtle">{t('revenueChart.transactions', { count: data.count })}</p>
            {data.growthPercent !== 0 && (
                <p className={data.growthPercent > 0 ? 'text-ui-accent-fg' : 'text-ui-danger'}>
...
const RevenueChart = ({ entries }: Props) => {
    ...
    return (
        <div className="rounded-ui-md border border-ui-border bg-ui-surface p-[18px]">
            <h3 className="mb-4 text-[15px] font-ui-bold text-ui-fg">{t('revenueChart.title')}</h3>
            <div className="h-64 w-full">
                <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={data}>
                        ...
                        />
                        <Tooltip content={<CustomTooltip />} cursor={{ stroke: 'var(--ui-tertiary)' }} />
                        <Area type="monotone" dataKey="revenueFormatted" stroke="var(--ui-accent-border)" strokeWidth={2} fill="url(#revenueGradient)" />
                    </AreaChart>
                </ResponsiveContainer>
            </div>
        </div>
