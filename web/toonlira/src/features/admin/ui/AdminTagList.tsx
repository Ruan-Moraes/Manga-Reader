...
const buildColumns = (t: TFunction, lang: LanguageTag, onEdit: (tag: AdminTag) => void, onDelete: (tag: AdminTag) => void): Column<AdminTag>[] => [
    {
        ...
        hideBelow: 'sm',
        render: tag => <span className="font-ui-mono text-ui-tiny text-ui-fg-subtle">{tag.value}</span>,
    },
    {
        key: 'label',
        header: t('tagList.columnName'),
        sortable: true,
        render: tag => <span className="font-ui-bold text-ui-fg">{tag.label?.[lang] ?? tag.label?.['pt-BR'] ?? ''}</span>,
    },
    {
        key: 'actions',
