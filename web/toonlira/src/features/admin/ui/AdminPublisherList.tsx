...
const buildColumns = (t: TFunction, onEdit: (p: AdminPublisher) => void, onDelete: (p: AdminPublisher) => void): Column<AdminPublisher>[] => [
    {
        ...
        sortable: true,
        render: publisher => <span className="font-ui-bold text-ui-fg">{publisher.name}</span>,
    },
    {
        key: 'slug',
        header: t('publisherList.columnSlug'),
        hideBelow: 'sm',
        render: publisher => <span className="font-ui-mono text-ui-tiny text-ui-fg-subtle">{publisher.slug}</span>,
    },
    {
        key: 'country',
        header: t('publisherList.columnCountry'),
        hideBelow: 'sm',
        render: publisher => <span className="text-ui-fg-subtle">{publisher.country ?? '—'}</span>,
    },
    {
        key: 'website',
        ...
        render: publisher =>
            publisher.website ? (
                <a
                    ...
                    className="font-ui-mono text-ui-tiny text-ui-accent-fg hover:underline"
                >
                    {publisher.website}
                </a>
            ) : (
    ...
    },
    {
        ...
        sortable: true,
        hideBelow: 'md',
        render: publisher => <span className="text-ui-fg-subtle">{formatDate(publisher.createdAt)}</span>,
    },
    {
        key: 'actions',
