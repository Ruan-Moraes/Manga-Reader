    ...
    return (
        ...
        >
            <div className="flex flex-col gap-3.5">
                {result ? (
                    <>
                        <p className="text-ui-small text-ui-fg">{t('dashboard.chapters.bulk.partialSummary', { ok: result.succeeded.length, failed: result.failed.length })}</p>
                        <ul className="flex max-h-[40vh] flex-col gap-1.5 overflow-y-auto">
                            {result.failed.map(failure => (
                                <li key={failure.id} className="rounded-ui-xs border border-ui-danger-border bg-ui-danger-15 px-3 py-2 text-ui-tiny text-ui-fg">
                                    <span className="font-ui-mono text-ui-fg-subtle">{failure.id.slice(0, 10)}</span>{' '}
                                    <span className="text-ui-danger">{t(`dashboard.chapters.errors.${failure.error.code}`, { ...failure.error })}</span>
                                </li>
                            ))}
                        </ul>
                    </>
