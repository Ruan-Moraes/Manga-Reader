...
const SortableRow = ({ chapter }: { chapter: AdminChapter }) => {
    ...
    return (
        <li
            ...
            className={cn(
                'flex items-center gap-3 rounded-ui-xs border border-ui-border bg-ui-surface px-3 py-2.5',
                isDragging && 'z-10 border-ui-accent-50 bg-ui-accent-25 shadow-ui-black',
            )}
        >
            <button type="button" {...attributes} {...listeners} className="cursor-grab text-ui-fg-muted hover:text-ui-accent-fg" aria-label={chapter.title}>
                <GripVertical size={16} />
            </button>
            <span className="w-12 tabular-nums text-ui-small font-ui-bold text-ui-accent-fg">#{chapter.number}</span>
            <span className="min-w-0 flex-1 truncate text-ui-small text-ui-fg">{chapter.title}</span>
        </li>
    );
};
...
const ReorderChaptersModal = ({ isOpen, onClose, chapters, isSubmitting, onConfirm }: ReorderChaptersModalProps) => {
    ...
    return (
        ...
        >
            <div>
                <p className="mb-3 text-ui-tiny text-ui-fg-subtle">{t('dashboard.chapters.reorder.hint')}</p>
