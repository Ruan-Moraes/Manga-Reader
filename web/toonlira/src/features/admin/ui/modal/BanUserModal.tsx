    ...
    return (
        ...
        >
            <div className="flex flex-col gap-4">
                <p className="text-ui-body leading-relaxed text-ui-fg-muted">
                    <Trans i18nKey="banUser.warning" ns="admin" values={{ name: userName }} components={{ b: <b className="text-ui-fg" /> }} />
                </p>

                <label className="flex flex-col gap-1.5">
                    <span className="text-ui-small font-ui-bold text-ui-fg-muted">{t('banUser.reasonLabel')}</span>
                    <Textarea value={reason} onChange={e => setReason(e.target.value)} placeholder={t('banUser.reasonPlaceholder')} rows={3} />
                </label>

                <label className="flex flex-col gap-1.5">
                    <span className="text-ui-small font-ui-bold text-ui-fg-muted">{t('banUser.durationLabel')}</span>
                    <Select
                        value={duration}
                ...
                {duration === 'temporary' && (
                    <label className="flex flex-col gap-1.5">
                        <span className="text-ui-small font-ui-bold text-ui-fg-muted">{t('banUser.daysLabel')}</span>
