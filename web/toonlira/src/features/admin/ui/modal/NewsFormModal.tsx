    ...
    return (
        ...
        >
            <div className="flex flex-col gap-4">
                ...
                {showPreview && (
                    <article className="overflow-hidden rounded-ui-sm border border-ui-border bg-ui-secondary" aria-label={t('newsForm.previewTitle')}>
                        {coverImage && (
                            <img
                                src={coverImage}
                        ...
                        <div className="mx-auto max-w-3xl space-y-4 p-5 sm:p-8">
                            <p className="text-ui-tiny font-ui-bold uppercase tracking-wider text-ui-accent-fg">
                                {categoryOptions.find(option => option.value === category)?.label || category}
                            </p>
                            <h2 className="text-ui-h2 font-ui-extrabold leading-tight text-ui-fg">
                                {title[contentTab] || ptTitle || t('newsForm.previewUntitled')}
                            </h2>
                            {subtitle[contentTab] && <p className="text-ui-body text-ui-fg-muted">{subtitle[contentTab]}</p>}
                            <div className="space-y-4 text-ui-body leading-relaxed text-ui-fg">
                ...
                {news && <section className="rounded-ui-xs border border-ui-border bg-ui-secondary p-4"><div className="flex flex-wrap items-center gap-2"><span className="rounded-ui-full border border-ui-border px-3 py-1 text-ui-tiny font-ui-bold text-ui-fg">{t(`newsForm.status.${news.status}`)}</span>{news.status !== 'PUBLISHED' && <Button type="button" size="sm" onClick={onPublish} disabled={isSubmitting}>{t('newsForm.publishNow')}</Button>}{news.status === 'PUBLISHED' && <Button type="button" size="sm" variant="ghost" onClick={onUnpublish} disabled={isSubmitting}>{t('newsForm.unpublish')}</Button>}{news.status !== 'DRAFT' && <Button type="button" size="sm" variant="ghost" onClick={onDraft} disabled={isSubmitting}>{t('newsForm.moveToDraft')}</Button>}</div><div className="mt-3 flex flex-col gap-2 sm:flex-row"><Input type="datetime-local" value={scheduledAt} onChange={event => setScheduledAt(event.target.value)} aria-label={t('newsForm.scheduleAt')} /><Button type="button" variant="ghost" disabled={!scheduledAt || isSubmitting} onClick={() => onSchedule?.(new Date(scheduledAt).toISOString())}>{t('newsForm.schedule')}</Button></div></section>}
                ...
                <div className="flex flex-col gap-1.5">
                    <span className="text-ui-small font-ui-bold text-ui-fg-muted">{t('newsForm.content', 'Conteúdo')}</span>
                    <div className="flex gap-1 border-b border-ui-border-subtle" role="tablist">
                        {SUPPORTED_LANGUAGES.map(lang => {
                            ...
                            return (
                                ...
                                >
                                    {lang}
                                    {filled && <span className="text-ui-accent-fg">●</span>}
                                </button>
                            );
                        })}
                    </div>
                ...
                <div className="rounded-ui-xs border border-ui-border bg-ui-secondary p-4">
                    <p className="mb-3 text-ui-small text-ui-fg-muted">{t('newsForm.coverTemporaryNotice')}</p>
                    {coverImage && <img src={coverImage} alt={coverAlt['pt-BR'] || ptTitle} className="mb-3 aspect-video w-full rounded-ui-xs object-cover" />}
                ...
                <div className="border-t border-ui-border pt-5">
                    <h3 className="mb-4 text-ui-h4 font-ui-extrabold text-ui-fg">{t('newsForm.seoSection')}</h3>
                    <div className="space-y-4">
                        ...
                        <div className="rounded-ui-xs border border-ui-border bg-ui-secondary p-4">
                            <p className="text-ui-tiny text-ui-accent-fg">{window.location.origin}/news/{slug || 'slug-da-noticia'}</p>
                            <p className="mt-1 text-ui-body font-ui-bold text-ui-fg">{seoTitle[contentTab] || title[contentTab] || ptTitle}</p>
                            <p className="mt-1 text-ui-small text-ui-fg-muted">{seoDescription[contentTab] || excerpt[contentTab]}</p>
                        </div>
                    </div>
                </div>
