...
const InfoRow = ({ label, children }: { label: string; children: React.ReactNode }) => (
    <div className="flex items-center justify-between gap-3 border-b border-ui-gray-900 py-2.5 last:border-b-0">
        <span className="text-ui-small text-ui-fg-subtle">{label}</span>
        <span className="text-right text-ui-small font-ui-bold text-ui-fg">{children}</span>
    </div>
);
            ...
            >
                {isLoading || !group ? (
                    <div className="flex flex-col gap-3 p-2">
                        <div className="h-16 animate-ui-pulse rounded-ui-sm bg-ui-gray-800" />
                        <div className="h-32 animate-ui-pulse rounded-ui-sm bg-ui-gray-800" />
                    </div>
                ) : (
                    <div className="flex flex-col gap-4">
                        <div className="flex items-center gap-4 border-b border-ui-gray-900 pb-4">
                            <Avatar name={name} size={56} />
                            <div className="min-w-0">
                                <div className="text-ui-h4 font-ui-extrabold text-ui-fg">{name}</div>
                                <div className="font-ui-mono text-ui-small text-ui-accent-fg">@{group.username}</div>
                                <div className="mt-2">
                        ...
                        <div className="grid gap-x-8 sm:grid-cols-2">
                            <div>
                                <InfoRow label={t('groupDetail.id')}>
                                    <code className="font-ui-mono text-ui-tiny text-ui-fg-subtle">{group.id}</code>
                                </InfoRow>
                        ...
                        <div>
                            <div className="mb-1.5 text-ui-tiny font-ui-extrabold uppercase tracking-[0.08em] text-ui-fg-subtle">{t('groupDetail.description')}</div>
                            <p className="text-ui-body leading-relaxed text-ui-fg-muted">{resolveLocalized(group.description, i18n.language) || '—'}</p>
                        </div>

                        <div>
                            <div className="mb-2.5 text-ui-tiny font-ui-extrabold uppercase tracking-[0.08em] text-ui-fg-subtle">
                                {t('groupDetail.membersCount', { count: group.members.length })}
                            </div>
                            {group.members.length === 0 ? (
                                <p className="text-ui-small text-ui-fg-subtle">{t('groupDetail.empty')}</p>
                            ) : (
                                <div className="flex flex-col gap-2">
                                    {group.members.map(member => (
                                        <div
                                            key={member.userId}
                                            className="flex items-center gap-2.5 rounded-ui-sm border border-ui-gray-900 bg-ui-surface-muted px-2.5 py-2"
                                        >
                                            <Avatar name={member.userName} size={32} />
                                            <span className="min-w-0 flex-1 truncate font-ui-bold text-ui-fg">{member.userName}</span>
                                            ...
                                            <button
                                                ...
                                                className={cn(
                                                    'flex size-8 items-center justify-center rounded-ui-xs border border-ui-border text-ui-fg-muted transition-colors',
                                                    'hover:border-ui-accent-50 hover:bg-ui-accent-25 hover:text-ui-accent-fg',
                                                )}
                                            >
                                                <Award size={15} />
                                            </button>
