    ...
    return [
        {
            ...
            hideBelow: 'md',
            render: sub => <span className="font-ui-mono text-ui-tiny text-ui-fg-subtle">{sub.id.slice(0, 8)}</span>,
        },
        {
            key: 'userId',
            header: t('dashboard.subscriptions.columnUserId'),
            hideBelow: 'md',
            render: sub => <span className="font-ui-mono text-ui-tiny text-ui-fg-subtle">{sub.userId.slice(0, 8)}</span>,
        },
        {
            key: 'planPeriod',
            ...
            render: sub => (
                <div className="flex flex-col">
                    <span className="font-ui-bold text-ui-fg">{periodLabels[sub.planPeriod] ?? sub.planPeriod}</span>
                    <span className="text-ui-tiny tabular-nums text-ui-fg-subtle">{formatPrice(sub.planPriceInCents)}</span>
                </div>
            ),
        },
        {
        ...
        },
        {
            ...
            sortable: true,
            hideBelow: 'sm',
            render: sub => <span className="text-ui-fg-subtle">{formatDate(sub.startDate)}</span>,
        },
        {
            key: 'endDate',
            ...
            hideBelow: 'md',
            sortable: true,
            render: sub => <span className="text-ui-fg-subtle">{formatDate(sub.endDate)}</span>,
        },
        {
            key: 'actions',
