    ...
    const columns: Column<AdminPlan>[] = [
        {
            ...
            sortable: true,
            render: plan => <span className="font-ui-bold text-ui-fg">{periodLabels[plan.period] ?? plan.period}</span>,
        },
        {
            key: 'priceInCents',
            header: t('planList.columnPrice'),
            sortable: true,
            render: plan => <span className="font-ui-bold tabular-nums text-ui-fg">{formatPrice(plan.priceInCents)}</span>,
        },
        {
            key: 'description',
