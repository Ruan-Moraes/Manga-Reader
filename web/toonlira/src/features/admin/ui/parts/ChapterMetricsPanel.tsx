...
const SeriesTooltip = ({ active, payload, label }: { active?: boolean; payload?: Array<{ value?: number | string }>; label?: string }) => {
    ...
    return (
        <div className="rounded-ui-xs border border-ui-border bg-ui-surface p-2.5 text-ui-small shadow-ui-black">
            <p className="font-ui-bold text-ui-fg">{label}</p>
            <p className="text-ui-fg-subtle">{Number(payload[0].value).toLocaleString(getLocale())}</p>
        </div>
    );
};
...
const ChapterMetricsPanel = ({ chapterId, titleId }: ChapterMetricsPanelProps) => {
    ...
    const renderBars = (data: { label: string; reads: number }[], titleKey: string) => (
        <div className="rounded-ui-md border border-ui-border bg-ui-surface p-[18px]">
            <h3 className="mb-4 text-[15px] font-ui-bold text-ui-fg">{t(titleKey)}</h3>
            <div className="h-44 w-full">
                <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={data} barCategoryGap="28%">
                        ...
                        <YAxis tick={{ fill: 'var(--ui-fg-subtle)', fontSize: 12 }} tickLine={false} />
                        <Tooltip content={<SeriesTooltip />} cursor={{ fill: 'var(--ui-surface-elevated)' }} />
                        <Bar dataKey="reads" fill="var(--ui-accent)" radius={[4, 4, 0, 0]} maxBarSize={42} />
                    </BarChart>
                </ResponsiveContainer>
            </div>
        </div>
    ...
    return (
        <div className="flex flex-col gap-4">
            ...
            <div className="rounded-ui-md border border-ui-border bg-ui-surface p-[18px]">
                <h3 className="mb-4 text-[15px] font-ui-bold text-ui-fg">{t('dashboard.chapters.metrics.readsEvolution')}</h3>
                        ...
                        <AreaChart data={analytics.series.map((point: SeriesPoint) => ({ ...point, label: formatDay(point.date) }))}>
                            <defs>
                                <linearGradient id="chapterReadsGradient" x1="0" y1="0" x2="0" y2="1">
                                    <stop offset="5%" stopColor="var(--ui-accent)" stopOpacity={0.12} />
                                    <stop offset="95%" stopColor="var(--ui-accent)" stopOpacity={0} />
                                </linearGradient>
                            </defs>
                            <CartesianGrid strokeDasharray="3 3" stroke="var(--ui-border)" />
                            <XAxis dataKey="label" tick={{ fill: 'var(--ui-fg-subtle)', fontSize: 12 }} tickLine={false} minTickGap={24} />
                            <YAxis tick={{ fill: 'var(--ui-fg-subtle)', fontSize: 12 }} tickLine={false} />
                            <Tooltip content={<SeriesTooltip />} cursor={{ stroke: 'var(--ui-tertiary)' }} />
                            <Area type="monotone" dataKey="value" stroke="var(--ui-accent-border)" strokeWidth={2} fill="url(#chapterReadsGradient)" />
                        </AreaChart>
                    </ResponsiveContainer>
                </div>
            </div>
            ...
            <div className="rounded-ui-md border border-ui-border bg-ui-surface p-5">
                <h3 className="mb-3 text-[15px] font-ui-bold text-ui-fg">{t('dashboard.chapters.metrics.detailsTitle')}</h3>
                <dl className="grid gap-x-8 gap-y-2.5 sm:grid-cols-2">
                    {comparisonRows.map(([label, value]) => (
                        <div key={label} className="flex items-baseline justify-between gap-4 border-b border-ui-gray-900 pb-2 last:border-b-0">
                            <dt className="text-ui-small text-ui-fg-subtle">{label}</dt>
                            <dd className="text-right text-ui-small font-ui-bold tabular-nums text-ui-fg">{value}</dd>
                        </div>
                    ))}
                </dl>
            </div>
