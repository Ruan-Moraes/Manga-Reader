    ...
    return (
        <div className="flex flex-col gap-2">
            {value.map((entry, index) => (
                <div key={`${entry.authorId}-${index}`} className="flex items-center gap-2 rounded-ui-sm border border-ui-gray-900 bg-ui-surface-muted px-2.5 py-2">
                    <span className="min-w-0 flex-1 truncate font-ui-bold text-ui-fg">{entry.authorName ?? `#${entry.authorId}`}</span>
                    ...
                    <button
                        ...
                        className={cn(
                            'flex size-8 shrink-0 items-center justify-center rounded-ui-xs border border-ui-danger-border text-ui-danger transition-colors hover:bg-ui-danger-15',
                        )}
                    >
                        <X size={15} />
                    </button>
