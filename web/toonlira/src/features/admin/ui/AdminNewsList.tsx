...
const buildColumns = (t: TFunction, lang: LanguageTag, onEdit: (news: AdminNews) => void, onDelete: (news: AdminNews) => void): Column<AdminNews>[] => [
    {
        ...
        hideBelow: 'md',
        render: news => <span className="font-ui-mono text-ui-tiny text-ui-fg-subtle">{news.id.slice(0, 8)}</span>,
    },
    {
        key: 'title',
        header: t('dashboard.news.columnTitle'),
        sortable: true,
        render: news => <span className="font-ui-bold text-ui-fg">{news.title?.[lang] ?? news.title?.['pt-BR'] ?? ''}</span>,
    },
    {
        key: 'category',
    ...
    },
    {
        ...
        sortable: true,
        hideBelow: 'md',
        render: news => <span className="tabular-nums text-ui-fg-subtle">{news.views.toLocaleString(getLocale())}</span>,
    },
    {
        key: 'isFeatured',
    ...
    },
    {
        ...
        sortable: true,
        hideBelow: 'md',
        render: news => <span className="text-ui-fg-subtle">{formatDate(news.publishedAt)}</span>,
    },
    {
        key: 'actions',
