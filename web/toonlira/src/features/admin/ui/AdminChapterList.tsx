...
const buildColumns = (
...
): Column<AdminChapter>[] => [
    {
        ...
        sortable: true,
        render: chapter => <span className="tabular-nums font-ui-bold text-ui-fg">{chapter.number}</span>,
    },
    {
        key: 'title',
        ...
        render: chapter => (
            <div className="min-w-0">
                <div className="truncate font-ui-bold text-ui-fg">{chapter.title}</div>
                <div className="truncate text-ui-tiny text-ui-fg-subtle">{chapter.titleName}</div>
            </div>
        ),
    },
    {
    ...
    },
    {
        ...
        sortable: true,
        hideBelow: 'md',
        render: chapter => <span className="text-ui-fg-subtle">{formatDate(chapter.publishedAt)}</span>,
    },
    {
        key: 'updatedAt',
        ...
        sortable: true,
        hideBelow: 'md',
        render: chapter => <span className="text-ui-fg-subtle">{formatDate(chapter.updatedAt)}</span>,
    },
    {
        key: 'reads',
        ...
        hideBelow: 'md',
        align: 'right',
        render: chapter => <span className="tabular-nums text-ui-fg-subtle">{chapter.reads.toLocaleString(getLocale())}</span>,
    },
    {
        key: 'completionRate',
        ...
        hideBelow: 'md',
        align: 'right',
        render: chapter => <span className="tabular-nums text-ui-fg-subtle">{chapter.reads > 0 ? `${Math.round(chapter.completionRate * 100)}%` : '—'}</span>,
    },
    {
        key: 'actions',
...
const AdminChapterList = ({
    ...
    return (
        <div className="flex flex-col gap-3">
            {selectedKeys.size > 0 && (
                <div className="flex flex-wrap items-center gap-2.5 rounded-ui-md border border-ui-accent-50 bg-ui-accent-25 px-4 py-2.5">
                    <span className="text-ui-small font-ui-bold text-ui-fg">{t('dashboard.chapters.bulk.selected', { count: selectedKeys.size })}</span>
