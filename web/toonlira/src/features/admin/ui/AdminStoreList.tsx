...
const AdminStoreList = ({ stores, page, totalPages, isLoading, onPageChange, onEdit, onDelete }: Props) => {
    ...
    const columns: Column<AdminStore>[] = [
        { key: 'name', header: t('stores.name'), render: store => <span className="font-ui-bold text-ui-fg">{store.name[i18n.language as keyof typeof store.name] ?? store.name['pt-BR'] ?? Object.values(store.name)[0]}</span> },
        { key: 'website', header: t('stores.website'), hideBelow: 'md', render: store => <span className="max-w-48 truncate text-ui-fg-subtle">{store.website}</span> },
        { key: 'status', header: t('stores.status'), render: store => <span className={store.status === 'ACTIVE' ? 'text-ui-success' : 'text-ui-fg-subtle'}>{store.status === 'ACTIVE' ? t('stores.active') : t('stores.inactive')}</span> },
