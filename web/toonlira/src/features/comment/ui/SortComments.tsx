...
const SortComments = ({ title }: SortCommentsProps) => {
    ...
    const segmentClass = (on: boolean) =>
        cn(
            '!min-h-0 !rounded-ui-full !border-0 !p-2 !shadow-none transition-colors duration-ui-default',
            on ? '!bg-ui-accent !text-ui-on-accent' : '!bg-transparent text-ui-fg-subtle hover:!bg-ui-accent-25 hover:text-ui-fg',
        );

    return (
        <div className="flex flex-wrap items-center gap-3">
            <span className="text-ui-small font-ui-semibold text-ui-fg-subtle">{title}</span>
            <div className="inline-flex gap-1 items-center rounded-ui-full border border-ui-chip-border bg-ui-chip p-1">
                <IconButton
                    icon={ArrowUpDown}
                    title="Ordem padrão"
                ...
                <span className="mx-1 h-4 w-px shrink-0 bg-ui-chip-border" aria-hidden="true" />
