        ...
        <div className="flex flex-col gap-3">
            ...
            {hasMore && (
                <button
                    type="button"
                    onClick={loadMore}
                    className="mt-4 self-center text-sm font-semibold text-ui-accent-fg hover:text-ui-accent-fg transition-colors cursor-pointer"
                >
                    {t('list.loadMore')}
                </button>
            )}
            {roots.length === 0 && !isLoading && !isError && <div className="mt-12 text-center font-bold text-ui-fg-muted">{t('list.empty')}</div>}
        </div>
    );
};
