    ...
    return (
        <header className={cn('mb-4 flex flex-col items-start gap-3 sm:flex-row sm:items-end sm:justify-between', className)}>
            <div className="min-w-0">
                {eyebrow && <div className="ui-label mb-1 text-ui-fg-subtle">{eyebrow}</div>}
                <TitleTag className={cn('m-0 font-ui-extrabold leading-tight tracking-mr text-ui-fg', sizeMap[size])}>{title}</TitleTag>
            </div>
            {(meta ?? action) && (
                <div className="flex w-full min-w-0 flex-wrap items-center gap-3 text-ui-tiny text-ui-fg-subtle sm:w-auto sm:justify-end">
                    {meta}
                    {action}
                </div>
            )}
