    ...
    return (
        <dialog
            ...
            className={cn(
                ...
                sizeMap[size],
                'rounded-ui-lg border border-ui-border bg-ui-surface text-ui-fg shadow-ui-black animate-ui-fade-in',
                '[&::backdrop]:bg-ui-overlay [&::backdrop]:backdrop-blur-mr',
            )}
            aria-labelledby={titleId}
            aria-describedby={description ? descriptionId : undefined}
        >
            <div ref={setToastPortalHost} className="pointer-events-none fixed inset-0 z-ui-toast" />
            <header className="flex shrink-0 items-start justify-between gap-4 border-b border-ui-border-subtle p-4 sm:px-6 sm:py-5">
                <div className="flex min-w-0 flex-1 items-start gap-3">
                    {Icon && (
                        <span
                            className={cn(
                                'mt-0.5 flex size-9 shrink-0 items-center justify-center rounded-ui-sm',
                                danger ? 'bg-ui-danger-15 text-ui-danger' : 'bg-ui-accent-10 text-ui-accent-fg',
                            )}
                            aria-hidden="true"
                        >
                    ...
                    <div className="min-w-0 flex-1">
                        {eyebrow && <div className="ui-label text-ui-accent-fg mb-1">{eyebrow}</div>}
                        <h2 id={titleId} className={`text-ui-body! sm:text-ui-h3! font-ui-extrabold! ${danger ? 'text-ui-danger' : 'text-ui-fg'}`}>
                            {title}
                        </h2>
                        {description && (
                            <p id={descriptionId} className="mt-1 text-ui-small text-ui-fg-subtle">
                                {description}
                            </p>
                        )}
                    </div>
            ...
            <FloatingPortalContext.Provider value={dialogEl}>
                <ModalRequestCloseContext.Provider value={requestClose}>
                    ...
                    {footer && (
                        <footer className="flex items-center justify-end gap-2 border-t border-ui-border-subtle bg-ui-surface-muted p-4 sm:px-6">{footer}</footer>
                    )}
            ...
            {confirmClose && (
                <dialog
                    ...
                    className="m-auto max-w-[420px] w-[90vw] overflow-hidden rounded-ui-lg border border-ui-border bg-ui-surface p-0 text-ui-fg shadow-ui-black animate-ui-fade-in [&::backdrop]:bg-ui-overlay [&::backdrop]:backdrop-blur-mr"
                    aria-labelledby={confirmTitleId}
                >
                    <div ref={setConfirmToastPortalHost} className="pointer-events-none fixed inset-0 z-ui-toast" />
                    <div className="p-5">
                        <h3 id={confirmTitleId} className="text-ui-h4! font-ui-extrabold! text-ui-fg">
                            {t('modal.unsavedTitle', 'Descartar alterações?')}
                        </h3>
                        <p className="mt-1.5 text-ui-small leading-relaxed text-ui-fg-subtle">
                            {t('modal.unsavedBody', 'Há alterações não salvas. Se você fechar agora, elas serão perdidas.')}
                        </p>
                    </div>
                    <footer className="flex items-center justify-end gap-2 border-t border-ui-border-subtle bg-ui-surface-muted p-4 sm:px-5">
