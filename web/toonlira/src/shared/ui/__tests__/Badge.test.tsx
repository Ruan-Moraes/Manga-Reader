...
describe('Badge', () => {
    ...
    it('variant accent por padrão', () => {
        ...
        expect(container.firstChild).toHaveClass('bg-ui-accent-25');
    });

    it('variant neutral', () => {
        const { container } = render(<Badge variant="neutral">Neutral</Badge>);
        expect(container.firstChild).toHaveClass('bg-ui-gray-800');
    });

    it('variant danger', () => {
        const { container } = render(<Badge variant="danger">Danger</Badge>);
        expect(container.firstChild).toHaveClass('text-ui-danger');
    });
