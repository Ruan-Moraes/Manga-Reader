...
describe('Select', () => {
    ...
    it('disabled', () => {
        ...
        expect(select).toHaveClass('bg-ui-surface-muted', 'border-ui-border-subtle', 'text-ui-fg-disabled');
    });
    ...
    it('uses the same control dimensions and open-state indicator as the other form fields', async () => {
        ...
        const trigger = screen.getByRole('combobox');
        expect(trigger).toHaveClass('h-11', 'rounded-ui-sm', 'border', 'border-ui-border');
...
describe('Select multiple', () => {
    ...
    it('uses a subtle selected state for options in the multi-select menu', async () => {
        ...
        expect(await screen.findByRole('menuitemcheckbox', { name: 'Opção A' })).toHaveClass('bg-ui-accent-10', 'text-ui-accent-fg');
    });
