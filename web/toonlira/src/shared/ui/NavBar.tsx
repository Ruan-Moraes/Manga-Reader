...
const Wordmark = ({ onClick }: { onClick?: () => void }) => (
    <button
        ...
        className="font-ui-sans font-ui-extrabold italic tracking-ui-logo text-ui-fg shrink-0"
    >
        {brand.name}
    </button>
);
...
export const NavBar = ({
    ...
    return (
        <header role="banner" className="sticky top-0 z-ui-header border-b-2 border-ui-tertiary bg-ui-primary">
            <div className="mx-auto flex h-16 max-w-ui-container items-center gap-3 px-4 sm:px-5 lg:px-6 md:h-[72px]">
                    ...
                    <nav aria-label="Principal" className="hidden md:flex items-center gap-1">
                        {links.map(l => (
                            <button
                                ...
                                type="button"
                                onClick={l.onClick}
                                className={`px-3 py-2 text-ui-small font-ui-bold rounded-ui-xs transition-colors duration-ui-default ${
                                    activeKey === l.key ? 'text-ui-accent-fg' : 'text-ui-fg-muted hover:text-ui-fg'
                                }`}
                            >
                                {l.label}
                            </button>
                        ...
                        <>
                            <div className="relative">
                                ...
                                {!!user.notificationCount && (
                                    <span className="absolute -right-1 -top-1 inline-flex h-4 min-w-4 items-center justify-center rounded-ui-full bg-ui-danger px-1 text-[10px] font-ui-extrabold text-ui-fg">
                            ...
                            <div className="relative">
                                ...
                                {!!user.libraryCount && (
                                    <span className="absolute -right-1 -top-1 inline-flex h-4 min-w-4 items-center justify-center rounded-ui-full bg-ui-accent px-1 text-[10px] font-ui-extrabold text-ui-on-accent">
                            ...
                            <DropdownMenu
                                trigger={
                                    <button type="button" aria-label={`Conta de ${user.name}`} className="ui-focus-ring">
            ...
            {mobileSearchOpen && (
                <div className="border-t border-ui-border-subtle px-4 py-2 md:hidden">
                    <SearchField
                        value={searchValue}
                        onChange={v => onSearchChange?.(v)}
                        onKeyDown={e => {
