                ...
                <>
                    {!!bottomToasts.length && (
                        <div
                            ...
                            aria-live="polite"
                            className="pointer-events-none fixed bottom-4 right-4 z-ui-toast flex w-[360px] max-w-[calc(100vw-2rem)] flex-col-reverse gap-2.5 max-sm:bottom-20 max-sm:left-1/2 max-sm:right-auto max-sm:-translate-x-1/2"
                        >
                            {bottomToasts.map(t => (
                    ...
                    {!!topToasts.length && (
                        <div
                            ...
                            aria-label="Notificações"
                            aria-live="polite"
                            className="pointer-events-none fixed top-4 right-4 z-ui-toast flex w-[360px] max-w-[calc(100vw-2rem)] flex-col gap-2.5 max-sm:left-1/2 max-sm:right-auto max-sm:-translate-x-1/2"
                        >
                            {topToasts.map(t => (
                                <ToastItem key={t.id} item={t} onDismiss={dismissToast} />
                            ))}
