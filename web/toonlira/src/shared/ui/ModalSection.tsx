    ...
    <section className={cn('flex flex-col gap-4 [&:not(:first-child)]:mt-7', className)}>
        {(title ?? description) && (
            <div className="flex flex-col gap-0.5">
                {title && (
                    <h3 className="ui-label flex items-center gap-3 text-ui-accent-fg after:h-px after:flex-1 after:bg-ui-border-subtle">{title}</h3>
                )}
                {description && <p className="text-ui-tiny text-ui-fg-subtle">{description}</p>}
            </div>
        )}
        {children}
    </section>
