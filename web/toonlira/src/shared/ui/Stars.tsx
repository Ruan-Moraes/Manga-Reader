...
const StarFill = ({ fillFraction, size }: { fillFraction: number; size: number }) => (
    <span aria-hidden="true" style={{ position: 'relative', display: 'inline-flex', width: size, height: size, flexShrink: 0 }}>
        ...
        <svg width={size} height={size} viewBox="0 0 24 24">
            <polygon points={STAR_POINTS} fill="var(--ui-gray-700)" />
        </svg>
        ...
        {fillFraction > 0 && (
            ...
            >
                <svg width={size} height={size} viewBox="0 0 24 24">
                    <polygon points={STAR_POINTS} fill="var(--ui-accent)" />
                </svg>
            </span>
        )}
    </span>
        ...
        <fieldset className="inline-flex min-w-0 shrink-0 gap-1 border-none p-0">
            ...
            {[1, 2, 3, 4, 5].map(i => (
                <button
                    ...
                    className="text-ui-gray-700 hover:text-ui-accent-fg ui-focus-ring"
                >
                    <svg width={size} height={size} viewBox="0 0 24 24" aria-hidden="true">
                        <polygon points={STAR_POINTS} fill={i <= shown ? 'var(--ui-accent)' : 'currentColor'} />
                    </svg>
                </button>
            ))}
        </fieldset>
