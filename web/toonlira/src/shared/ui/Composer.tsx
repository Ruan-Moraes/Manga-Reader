            ...
            <div
                className={
                    bare
                        ...
                        : cn(
                            'grid gap-3 rounded-ui-xs border bg-ui-surface p-3.5 transition-colors md:grid-cols-[auto_1fr]',
                            focused ? 'border-ui-accent-50 shadow-ui-elevated' : 'border-ui-border',
                        )
                }
            >
                ...
                <div className={cn('flex min-w-0 flex-col gap-2.5', fill && 'flex-1')}>
                    {/* abas */}
                    <div className="flex gap-0.5 border-b border-ui-border">
                        {(['write', 'preview'] as const).map(key => (
                            <button
                                ...
                                onClick={() => setTab(key)}
                                className={cn(
                                    '-mb-px whitespace-nowrap border-b-2 px-3 py-[7px] text-[12.5px] font-ui-bold transition-colors max-mobile-md:px-2',
                                    tab === key ? 'border-ui-accent-border text-ui-accent-fg' : 'border-transparent text-ui-fg-subtle hover:text-ui-fg',
                                )}
                            >
                    ...
                    {tab === 'write' ? (
                        <>
                            <div className="flex flex-wrap items-center gap-0.5">
                                {tools.map(tool =>
                                    'sep' in tool ? (
                                        <span key={tool.key} className="mx-1 h-[18px] w-px bg-ui-border" />
                                    ) : (
                                        <button
                                            key={tool.key}
                                            type="button"
                                            ...
                                            onClick={tool.run}
                                            className="grid size-[30px] place-items-center rounded-ui-xs border border-transparent text-ui-fg-subtle transition-all hover:border-ui-chip-border hover:bg-ui-chip hover:text-ui-fg"
                                        >
                            ...
                            <textarea
                                ...
                                className={cn(
                                    'min-h-24 w-full resize-y rounded-xs border border-ui-border bg-ui-surface-muted px-3 py-3 text-ui-body leading-relaxed text-ui-fg placeholder:text-ui-tertiary focus:border-ui-accent-border focus:outline-none',
                                    fill && 'flex-1 resize-none',
                                )}
                            />
                        </>
                    ) : (
                        <div className={cn('min-h-24 overflow-y-auto rounded-xs border border-ui-border bg-ui-surface-muted px-3 py-3', fill && 'flex-1')}>
                            {value.trim() ? (
                                <Markdown text={value} className="text-ui-body leading-[1.62] text-ui-fg-muted" />
                            ) : (
                                <span className="italic text-ui-tertiary">{t('composer.previewEmpty')}</span>
                            )}
                        </div>
                    )}
                            ...
                            {images!.map((src, index) => (
                                <div key={src} className="relative inline-block">
                                    <img src={src} alt={t('composer.imageAlt', { index: index + 1 })}
                                         className="max-h-40 rounded-ui-xs object-cover" />
                                    <button
                                        type="button"
                                        onClick={() => onRemoveImage?.(index)}
                                        aria-label={t('composer.removeImage')}
                                        className="absolute right-0 top-0 rounded-bl-xs rounded-tr-xs bg-ui-danger px-1.5 py-0.5 text-xs text-ui-on-overlay opacity-75 hover:opacity-100"
                                    >
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-3 max-md:justify-end">
                        <span
                            className="inline-flex flex-wrap items-center gap-1.5 text-[11.5px] text-ui-tertiary max-md:hidden">
                            <HelpCircle className="size-3" aria-hidden="true" />
                            {t('composer.hint')}
                        </span>
                        <div className="flex items-center gap-2.5">
                            <span className="text-[11.5px] tabular-nums text-ui-tertiary">
                            ...
                            {onCancel && (
                                <button
                                    ...
                                    className="inline-flex py-2.5 items-center gap-1.5 rounded-ui-xs border border-ui-chip-border bg-ui-chip px-[13px] text-[12.5px] font-ui-bold text-ui-fg-subtle transition-all hover:border-ui-accent-50 hover:text-ui-fg max-md:justify-center max-md:gap-0 max-md:px-3"
                                >
                            ...
                            <button
                                ...
                                className="inline-flex py-2.5 items-center gap-1.5 rounded-ui-xs border border-ui-accent-border bg-ui-accent px-[18px] text-[13px] font-ui-extrabold text-ui-on-accent transition-opacity hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-40 max-md:justify-center max-md:px-3"
                            >
