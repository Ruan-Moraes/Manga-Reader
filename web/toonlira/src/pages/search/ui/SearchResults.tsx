...
const SearchResults = () => {
    ...
    return (
        <PageContainer asMain size="default" paddingY="md">
            <div className="relative mb-7 overflow-hidden rounded-ui-lg border border-ui-separator bg-ui-surface px-4 py-5 shadow-sm sm:px-6 sm:py-7">
                <div className="pointer-events-none absolute -right-10 -top-16 size-48 rounded-ui-full bg-ui-accent-10 blur-3xl" />
            ...
            {isValid && (
                <div className="mb-8 overflow-x-auto pb-1">
                    <div className="flex w-max gap-2 rounded-ui-full border border-ui-separator bg-ui-surface p-1" role="tablist" aria-label={t('search.filtersAria')}>
                        {TYPES.map(item => {
                            ...
                            return (
                                <button
                                    ...
                                    className={cn(
                                        'ui-focus-ring inline-flex shrink-0 items-center gap-1.5 rounded-ui-full px-3 py-2 text-ui-small font-ui-bold motion-safe:transition-all motion-safe:duration-ui-default',
                                        type === item ? 'bg-ui-primary text-ui-on-primary shadow-sm' : 'text-ui-fg-muted hover:bg-ui-accent-10 hover:text-ui-fg',
                                    )}
                                >
                ...
                <div className="space-y-9">
                    {overview.data.sections.map(section => {
                        ...
                        <section key={section.type} aria-labelledby={`search-section-${section.type}`}>
                            <div className="mb-4 flex items-center justify-between gap-4 border-b border-ui-separator pb-3">
                                <div className="flex items-center gap-3">
                                    <span className="flex size-9 items-center justify-center rounded-ui-sm bg-ui-accent-10 text-ui-accent-fg">
                                        <Icon className="size-4" aria-hidden="true" />
                                    </span>
                                    <div>
                                    <h2 id={`search-section-${section.type}`} className="text-xl font-ui-extrabold text-ui-fg">{t(`search.type.${section.type}`)}</h2>
                                    <p className="text-ui-small text-ui-fg-muted">{t('search.categoryCount', { count: section.totalElements, category: t(`search.filter.${section.type}`) })}</p>
                                    </div>
                                </div>
...
const ResultsSkeleton = () => (
    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3" aria-hidden="true">
        {Array.from({ length: 9 }).map((_, index) => (
            <div key={index} className="flex gap-4 rounded-ui-xs border border-ui-separator p-3">
                <Skeleton variant="rect" width={64} height={80} className="rounded-ui-xs" />
