    ...
    return (
        <section aria-label={t('hero.ariaLabel')} onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)}>
            {hero ? (
                        ...
                        <div className="mt-4 flex justify-center gap-2" role="tablist" aria-label={t('hero.indicators')}>
                            {featuredTitles.map((title, i) => (
                                <button
                                    ...
                                    className={`h-1.5 rounded-ui-full transition-all duration-ui-default ${
                                        i === heroIdx
                                            ? 'w-6 bg-ui-accent-border'
                                            : 'w-1.5 bg-ui-fg-subtle/60 hover:bg-ui-accent-border/70'
                                    }`}
                                />
                            ))}
                        </div>
                ...
                <Skeleton variant="rect" height={400} className="rounded-ui-xs" />
            )}
        </section>
    );
};
