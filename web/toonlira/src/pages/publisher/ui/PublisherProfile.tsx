        ...
        <PageContainer asMain size="wide" paddingY="lg">
            <header className="relative overflow-hidden rounded-ui-lg border border-ui-separator bg-ui-surface p-5 shadow-sm sm:p-7">
                ...
                <div className="min-w-0 flex-1 sm:pt-1">
                    <p className="inline-flex items-center gap-1.5 text-ui-tiny font-ui-extrabold uppercase tracking-widest text-ui-accent-fg">
                        <Building2 className="size-3.5" aria-hidden="true" />
                        {t('publisher.eyebrow')}
                    </p>
                    <h1 className="mt-1 break-words text-2xl font-ui-extrabold text-ui-fg sm:text-3xl">{publisher.name}</h1>
                    <p className="mt-2 inline-flex items-center gap-1.5 text-ui-small text-ui-fg-muted">
                    ...
                    {websiteUrl && (
                        <a href={websiteUrl} target="_blank" rel="noreferrer" className="ui-focus-ring mt-4 inline-flex items-center gap-1.5 rounded-ui-xs border border-ui-accent-border bg-ui-accent-10 px-3 py-2 text-ui-small font-ui-bold text-ui-accent-fg motion-safe:transition-colors hover:bg-ui-accent-25">
                ...
                {works && (
                    <div className="flex shrink-0 items-center gap-3 rounded-ui-sm border border-ui-separator bg-ui-primary px-4 py-3 sm:self-center">
                        ...
                        <div>
                            ...
                            <div className="text-ui-tiny font-ui-bold uppercase tracking-[0.08em] text-ui-fg-muted">{t('publisher.workCount', { count: works.totalElements })}</div>
                        </div>
                    </div>
                )}
                </div>
            ...
            <section className="py-7">
                <h2 className="mb-1 text-xl font-ui-extrabold text-ui-fg">{t('publisher.publishedWorks')}</h2>
                {works && <p className="mb-5 text-ui-small text-ui-fg-muted">{t('publisher.workCount', { count: works.totalElements })}</p>}
...
const WorksSkeleton = () => (
    <div className="grid grid-cols-2 gap-x-4 gap-y-7 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5" aria-hidden="true">
        {Array.from({ length: 10 }).map((_, index) => (
            <div key={index}><Skeleton variant="rect" height={220} className="rounded-ui-xs" /><Skeleton variant="text" lines={2} className="mt-2" /></div>
        ))}
    </div>
);
