    ...
    const columns: Column<ChapterMetricsSummaryRow>[] = [
        {
            ...
            render: row => <span className="tabular-nums font-ui-bold text-ui-fg">{row.chapterNumber}</span>,
        },
        {
            key: 'chapterTitle',
            ...
            render: row => (
                <div className="min-w-0">
                    <div className="truncate font-ui-bold text-ui-fg">{row.chapterTitle}</div>
                    <div className="truncate text-ui-tiny text-ui-fg-subtle">{row.titleName}</div>
                </div>
            ),
        },
        {
            ...
            align: 'right',
            render: row => <span className="tabular-nums text-ui-fg">{row.totalReads.toLocaleString(getLocale())}</span>,
        },
        {
            key: 'uniqueReaders',
            ...
            align: 'right',
            hideBelow: 'sm',
            render: row => <span className="tabular-nums text-ui-fg-subtle">{row.uniqueReaders.toLocaleString(getLocale())}</span>,
        },
        {
            key: 'completionRate',
            ...
            align: 'right',
            hideBelow: 'md',
            render: row => <span className="tabular-nums text-ui-fg-subtle">{Math.round(row.completionRate * 100)}%</span>,
        },
        {
            key: 'avgReadTimeSec',
            ...
            align: 'right',
            hideBelow: 'md',
            render: row => <span className="tabular-nums text-ui-fg-subtle">{Math.floor(row.avgReadTimeSec / 60)}m</span>,
        },
    ];
    ...
    return (
        <div className="flex flex-col gap-4">
            <div className="flex flex-wrap items-center gap-3">
                ...
                <div>
                    <h1 className="text-[26px] font-ui-extrabold leading-tight text-ui-fg md:text-[30px]">{t('dashboard.chapters.metrics.overviewTitle')}</h1>
                    <p className="mt-1.5 text-ui-small text-ui-fg-subtle">{t('dashboard.chapters.metrics.overviewSubtitle')}</p>
                </div>
            </div>
