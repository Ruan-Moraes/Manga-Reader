        ...
        <div className="flex flex-col gap-5">
            <div className="flex flex-wrap items-center gap-3">
                ...
                <div className="min-w-0 flex-1">
                    <h1 className="truncate text-[24px] font-ui-extrabold leading-tight text-ui-fg md:text-[28px]">
                        <span className="text-ui-accent-fg">#{chapter.number}</span> {chapter.title}
                    </h1>
                    <p className="mt-1 text-ui-small text-ui-fg-subtle">{chapter.titleName}</p>
                </div>
                    ...
                    <dl className="grid gap-x-8 gap-y-3 sm:grid-cols-2">
                        {infoRows.map(([label, value]) => (
                            <div key={label} className="flex items-baseline justify-between gap-4 border-b border-ui-gray-900 pb-2 last:border-b-0">
                                ...
                                <dd className="text-right text-ui-small font-ui-bold text-ui-fg">{value}</dd>
                            </div>
                        ))}
                    </dl>
                    {chapter.description && (
                        <div className="mt-4 rounded-ui-sm border border-ui-border bg-ui-surface-muted p-3.5">
                            <p className="mb-1 text-ui-tiny font-ui-bold uppercase tracking-mr text-ui-fg-subtle">{t('dashboard.chapters.form.description')}</p>
                            <p className="text-ui-small leading-relaxed text-ui-fg-muted">{chapter.description}</p>
                        </div>
                    )}
                </div>
            )}
