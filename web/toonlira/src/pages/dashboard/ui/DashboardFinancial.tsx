...
const DashboardFinancial = () => {
    ...
    return (
        <div className="flex flex-col gap-4">
            <div>
                <h1 className="text-[26px] font-ui-extrabold leading-tight text-ui-fg md:text-[30px]">{t('dashboard.financial.title')}</h1>
                <p className="mt-1.5 text-ui-small text-ui-fg-subtle">{t('dashboard.financial.subtitle')}</p>
            </div>

            {isLoadingSummary ? (
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                    {Array.from({ length: 3 }).map((_, i) => (
                        <div key={i} className="h-24 animate-ui-pulse rounded-ui-md bg-ui-gray-800" />
                    ))}
                </div>
            ) : isErrorSummary || !summary ? (
                <p className="text-ui-small text-ui-fg-subtle">{t('dashboard.financial.errorSummary')}</p>
            ) : (
                <FinancialDashboard summary={summary} />
            )}

            {isLoadingRevenue ? (
                <div className="h-64 animate-ui-pulse rounded-ui-md bg-ui-gray-800" />
            ) : revenueSeries ? (
                <>
            ...
            <div className="mt-3 flex flex-wrap items-end justify-between gap-2">
                <h2 className="text-[18px] font-ui-bold tracking-mr text-ui-fg">{t('dashboard.financial.payments')}</h2>
                <span className="text-ui-small text-ui-fg-subtle">{t('dashboard.financial.count', { count: totalElements })}</span>
            </div>

            <div className="flex flex-wrap items-center gap-2.5">
                <span className="text-ui-small font-ui-bold text-ui-fg-subtle">{t('dashboard.financial.statusLabel')}</span>
