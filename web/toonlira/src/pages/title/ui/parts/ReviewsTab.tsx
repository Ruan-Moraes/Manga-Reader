    ...
    return (
        <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-3">
                ...
                <span className="text-[13px] text-ui-fg-subtle">{t('reviews.reviewsCount', { count })}</span>
            </div>
...
function LoginPrompt({ onLogin, onRegister }: { onLogin: () => void; onRegister: () => void }) {
    ...
    return (
        <div className="mb-4 flex flex-col items-center gap-4 rounded-ui-lg border border-ui-border bg-ui-surface-interactive p-6 text-center sm:flex-row sm:text-left">
            <Illustration type="pensando" alt="" width={92} height={92} className="shrink-0" />
            <div className="min-w-0">
                <h3 className="text-[16px] font-ui-extrabold text-ui-fg">{t('reviews.login.title')}</h3>
                <p className="mt-1 text-[14px] text-ui-fg-muted">{t('reviews.login.desc')}</p>
