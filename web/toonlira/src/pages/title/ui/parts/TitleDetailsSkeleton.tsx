...
const shimmer = 'animate-pulse rounded bg-ui-tertiary/20';
...
const TitleDetailsSkeleton = () => (
    <PageContainer asMain size="default" paddingY="md">
        ...
        <div className="mb-6 flex flex-col gap-6 md:flex-row md:gap-8">
            <div className={`h-[220px] w-[156px] shrink-0 self-start rounded-ui-xs ${shimmer}`} />

            <div className="flex-1 min-w-0 space-y-3 py-1">
                {/* Badges de gênero */}
                <div className="flex flex-wrap gap-1.5">
                    {Array.from({ length: 4 }).map((_, i) => (
                        <div key={i} className={`h-5 w-16 rounded-ui-full ${shimmer}`} />
                    ))}
                </div>
                ...
                <div className="flex flex-wrap gap-2 pt-1">
                    <div className={`h-10 w-36 rounded-ui-xs ${shimmer}`} />
                    <div className={`h-10 w-32 rounded-ui-xs ${shimmer}`} />
                    <div className={`h-10 w-10 rounded-ui-xs ${shimmer}`} />
                </div>
            </div>
        </div>

        {/* Barra de abas (espelha <Tabs>) */}
        <div className="mb-6 flex gap-4 border-b border-ui-border pb-2">
        ...
        <div className="flex flex-col gap-3">
            {Array.from({ length: 5 }).map((_, i) => (
                <div key={i} className={`h-14 w-full rounded-ui-xs ${shimmer}`} />
            ))}
        </div>
    </PageContainer>
);
