    ...
    return (
        <div className="mb-6 flex flex-col gap-6 md:flex-row md:gap-8">
            <button
                ...
                className="shrink-0 self-start cursor-zoom-in rounded-ui-xs ui-focus-ring"
            >
            ...
            <div className="flex-1 min-w-0">
                ...
                <h1 className="mb-1 text-ui-h1 font-ui-extrabold tracking-mr text-ui-fg leading-tight">{title.name}</h1>
                <p className="mb-3 text-ui-small text-ui-fg-muted">
                ...
                >
                    ...
                    <span className="text-ui-small font-ui-bold text-ui-fg">{average.average.toFixed(1)}</span>
                    <span className="text-ui-tiny text-ui-fg-subtle">
                        · {average.count.toLocaleString()} {t('titleDetails.votes')}
                    </span>
                </div>

                <section aria-label={t('titleDetails.synopsisAria')} className="mb-5">
                    <p className={`text-ui-small leading-relaxed text-ui-fg-muted ${!expanded ? 'line-clamp-4 md:line-clamp-none' : ''}`}>{title.synopsis}</p>
                    <button type="button" onClick={() => setExpanded(e => !e)} className="mt-1 text-ui-tiny text-ui-accent-fg hover:underline md:hidden">
                ...
                <div className="flex flex-wrap gap-4 text-ui-tiny text-ui-fg-subtle">
                    {title.chaptersCount != null && (
                        <span>
                            <strong className="text-ui-fg">{title.chaptersCount}</strong> {t('titleDetails.chaptersLabel')}
                        </span>
                    )}
                    <span>
                        <strong className="text-ui-fg">{groupCount}</strong> {t('titleDetails.groupsLabel')}
                    </span>
                </div>
            </div>
        </div>
