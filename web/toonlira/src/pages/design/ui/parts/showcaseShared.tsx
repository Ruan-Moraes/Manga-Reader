export const Section = ({ title, children }: { title: string; children: React.ReactNode }) => (
    <section className="mb-10">
        <h2 className="mb-4 border-b border-ui-gray-700 pb-2 text-ui-h4 text-ui-fg-muted uppercase tracking-[0.1em]">{title}</h2>
...
export const SubSection = ({ label, children }: { label: string; children: React.ReactNode }) => (
    <div className="w-full">
        <p className="mb-2 text-ui-tiny text-ui-fg-muted uppercase tracking-widest">{label}</p>
        <div className="flex flex-wrap items-center gap-3">{children}</div>
    </div>
);
...
export const PhaseHeader = ({ title }: { title: string }) => (
    <>
        <h1 className="mb-2 mt-12 text-ui-h1">{title}</h1>
        <div className="mb-10 h-px bg-ui-border" />
    </>
);
