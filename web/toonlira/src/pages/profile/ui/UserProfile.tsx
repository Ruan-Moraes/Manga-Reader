    ...
    if (loading) {
        return (
            <PageContainer asMain size="default" paddingY="md">
                <Skeleton className="mb-6 h-[260px] w-full rounded-ui-xs" />
                <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                    {Array.from({ length: 6 }).map((_, i) => (
                        <Skeleton key={i} className="aspect-[2/3] w-full rounded-ui-xs" />
                    ))}
                </div>
            </PageContainer>
        );
        ...
        <PageContainer asMain size="default" paddingY="md">
            ...
            {tab === 'overview' && (
                <div className="flex flex-col gap-8">
                    <section>
                        <p className="ui-label mb-3 text-ui-fg-subtle">{t('profile.stats.reading')}</p>
                        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                            {readingNow.map(m => (
                                <MangaCard
                                    key={m.id}
                    ...
                    {recommendations.length > 0 && (
                        <section>
                            <p className="ui-label mb-3 text-ui-fg-subtle">{t('profile.tabs.recommendations')}</p>
                            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                    ...
                    <section>
                        <p className="ui-label mb-3 text-ui-fg-subtle">{t('profile.tabs.reviews')}</p>
                    ...
                    {groupsFollowed.length > 0 && (
                        <section>
                            <p className="ui-label mb-3 text-ui-fg-subtle">{t('profile.tabs.groups', { defaultValue: 'Grupos' })}</p>
