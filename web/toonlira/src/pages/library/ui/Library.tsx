            ...
            {loading ? (
                ...
                <div className="flex flex-col gap-2">
                    {sorted.map(m => (
                        <div
                            key={m.titleId}
                            className="flex items-center gap-3 rounded-ui-xs border border-ui-border bg-ui-surface px-4 py-3 transition-colors hover:border-ui-accent-border"
                        >
                            <button type="button" className="flex flex-1 items-center gap-3 text-left" onClick={() => navigate(ROUTES.TITLE_DETAIL(m.titleId))}>
                                <div
                                    className="size-12 shrink-0 rounded-ui-xs bg-cover bg-center bg-ui-tertiary/20"
                                    style={
                                        m.cover
                                            ? {
                                ...
                                <div className="flex-1 min-w-0">
                                    <p className="truncate text-ui-body font-ui-bold text-ui-fg">{m.name}</p>
                                    <p className="text-ui-tiny text-ui-fg-muted">{m.type}</p>
                                </div>
                                <span className="text-ui-tiny text-ui-fg-subtle">{m.list}</span>
                            </button>
                            <Button variant="ghost" size="sm" onClick={() => removeFromSaved(m.titleId)} className="text-ui-danger shrink-0">
                                {t('page.remove')}
                            </Button>
                        </div>
                    ))}
