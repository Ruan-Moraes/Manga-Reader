...
const Cell = ({ value, label, accent }: { value: number; label: string; accent: string }) => (
    <div style={{ textAlign: 'center', minWidth: 44 }}>
        <div style={{ fontSize: 18, fontWeight: 800, color: 'var(--ui-on-overlay)', fontVariantNumeric: 'tabular-nums', letterSpacing: '.0625rem' }}>{String(value).padStart(2, '0')}</div>
...
export const CountdownBox = ({ targetIso, accent }: { targetIso: string; accent: string }) => {
    ...
    const sep = <div style={{ color: 'var(--ui-border)', alignSelf: 'center' }}>:</div>;

    return (
        <div style={{ display: 'inline-flex', gap: 4, padding: '10px 12px', background: 'var(--ui-overlay-control)', border: `1px solid ${accent}33`, borderRadius: 4, backdropFilter: 'blur(8px)' }}>
