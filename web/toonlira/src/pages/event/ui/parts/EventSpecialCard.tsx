...
export const EventSpecialCard = ({ event, onOpen }: { event: CommunityEvent; onOpen: (id: string) => void }) => {
    ...
    const accent = event.accent || 'var(--ui-accent)';
    ...
    return (
        <div
            ...
            style={{
                position: 'relative', cursor: 'pointer', background: 'var(--ui-surface-muted)',
        ...
        >
            <div style={{ position: 'relative', height: 180, background: event.cover, overflow: 'hidden' }}>
                ...
                <div style={{ position: 'absolute', top: 14, left: 14, display: 'flex', gap: 8, alignItems: 'center' }}>
                    <span
                        style={{
                            display: 'inline-flex', alignItems: 'center', gap: 6, background: 'var(--ui-overlay-strong)', color: accent,
                ...
                <div style={{ position: 'absolute', left: 18, right: 18, bottom: 14 }}>
                    <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--ui-on-overlay)', letterSpacing: '.1em', textTransform: 'uppercase', marginBottom: 4 }}>{event.tagline}</div>
                    <h3 style={{ fontSize: 'clamp(18px,3vw,24px)', fontWeight: 800, color: 'var(--ui-on-overlay)', margin: 0, letterSpacing: '.0625rem', lineHeight: 1.15, textShadow: '0 2px 12px var(--ui-overlay)' }}>{event.name}</h3>
                </div>
            </div>

            <div style={{ padding: '18px 18px 20px' }}>
                <p style={{ fontSize: 13, color: 'var(--ui-fg-muted)', lineHeight: 1.6, margin: '0 0 14px', letterSpacing: '.0625rem' }}>{event.description}</p>

                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, alignItems: 'center', marginBottom: 14 }}>
                    <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 12, color: 'var(--ui-fg-subtle)' }}>
                        <span style={{ color: accent }}>▸</span>
                        {fmtRange(event.start, event.end)}
                    </div>
                    {!!event.chapters && event.chapters > 0 && <div style={{ fontSize: 12, color: 'var(--ui-fg-subtle)' }}>{t('ui.chaptersCount', { count: event.chapters })}</div>}
                    {!!event.participants && event.participants > 0 && <div style={{ fontSize: 12, color: 'var(--ui-fg-subtle)' }}>{t('ui.participantsCount', { count: event.participants })}</div>}
                </div>

                {(isActive || isUpcoming) && (
                    <div style={{ marginBottom: 14 }}>
                        <div style={{ fontSize: 10, fontWeight: 700, color: 'var(--ui-fg-disabled)', textTransform: 'uppercase', letterSpacing: '.12em', marginBottom: 6 }}>{isActive ? t('ui.endsIn') : t('ui.startsIn')}</div>
                ...
                <button
                    ...
                    style={{
                        width: '100%', padding: '12px 16px', background: isActive ? accent : 'transparent', color: isActive ? 'var(--ui-on-accent)' : 'var(--ui-fg)',
