...
const News = () => {
    ...
    return (
        <main className="mx-auto w-full max-w-[1440px] px-4 py-8 sm:px-6 lg:px-10 lg:py-12">
            <header className="mb-8 grid gap-6 border-b border-ui-border pb-8 lg:grid-cols-[1fr_auto] lg:items-end">
                <div className="max-w-3xl"><p className="mb-3 flex items-center gap-2 text-ui-tiny font-ui-bold uppercase tracking-[0.14em] text-ui-accent-fg"><Newspaper className="size-4" />{t('page.eyebrow')}</p><h1 className="text-ui-h1 font-ui-extrabold leading-none tracking-mr text-ui-fg">{t('page.title')}</h1><p className="mt-4 max-w-2xl text-ui-body leading-relaxed text-ui-fg-muted">{t('page.description')}</p></div>
                <p className="text-ui-small text-ui-fg-subtle" aria-live="polite">{t('page.results', { count: state.totalElements })}</p>
            </header>

            <form className="mb-5 flex gap-2" onSubmit={event => { event.preventDefault(); state.update({ q: search }); }} role="search">
                <div className="relative flex-1"><Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-ui-fg-subtle" /><Input value={search} onChange={event => setSearch(event.target.value)} placeholder={t('page.searchPlaceholder')} className="min-h-11 pl-10" aria-label={t('page.searchPlaceholder')} /></div>
            ...
            <section className={`${mobileFilters ? 'grid' : 'hidden'} mb-7 gap-4 rounded-ui-xs border border-ui-border bg-ui-surface p-4 lg:grid lg:grid-cols-[1fr_180px_180px]`} aria-label={t('page.filters')}>
                <div className="flex gap-2 overflow-x-auto pb-1">
                    <button className={`min-h-10 shrink-0 rounded-ui-full px-4 text-ui-small font-ui-bold ${!state.category ? 'bg-ui-accent text-ui-on-accent' : 'border border-ui-border text-ui-fg-muted'}`} onClick={() => state.update({ category: '' })} type="button">{t('page.allCategories')}</button>
                    {state.categories.map(option => <button key={option.value} type="button" onClick={() => state.update({ category: option.value })} className={`min-h-10 shrink-0 rounded-ui-full px-4 text-ui-small font-ui-bold ${state.category === option.value ? 'bg-ui-accent text-ui-on-accent' : 'border border-ui-border text-ui-fg-muted'}`}>{option.label}</button>)}
                </div>
