    ...
    return (
        <PageContainer asMain size="default" paddingY="md">
            <header className="mb-[18px]">
                <div className="mb-1.5 text-ui-tiny font-ui-extrabold uppercase tracking-[0.1em] text-ui-accent-fg">{t('page.eyebrow')}</div>
                <h1 className="m-0 text-[clamp(22px,4vw,28px)] font-ui-bold tracking-mr text-ui-fg">{t('page.title')}</h1>
                <p className="mt-1.5 max-w-[560px] text-ui-small leading-normal text-ui-gray-200">{t('page.sub')}</p>
            </header>

            <div className="mb-[18px] flex flex-wrap gap-2">
                <div className="relative min-w-[220px] flex-1">
                    <Search
                        className="pointer-events-none absolute left-2.5 top-1/2 size-3.5 -translate-y-1/2 text-ui-tertiary"
                        strokeWidth={2}
                        aria-hidden="true"
                    />
                    <input
                        ...
                        className="h-10 w-full rounded-ui-xs border border-ui-gray-700 bg-ui-secondary pl-8 pr-3 text-ui-small tracking-mr text-ui-fg outline-none focus:border-ui-accent-border"
                    />
                </div>
            ...
            {isLoading ? (
                <div className="grid gap-3.5" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(366px, 1fr))' }}>
                    {Array.from({ length: 6 }, (_, i) => (
                        <div key={i} className="h-[230px] animate-ui-pulse rounded-ui-sm border border-ui-border bg-ui-gray-900" />
                    ))}
                </div>
            ) : (
