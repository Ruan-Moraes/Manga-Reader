...
const DataRow = ({ title, desc, children }: { title: string; desc?: string; children: React.ReactNode }) => (
    <div className="flex flex-col gap-3 rounded-ui-xs border border-ui-gray-800 bg-ui-secondary p-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0">
            <p className="text-ui-body font-ui-bold text-ui-fg">{title}</p>
            {desc && <p className="mt-0.5 text-ui-small text-ui-fg-subtle">{desc}</p>}
        </div>
...
const DataTab = ({ state }: { state: SettingsState }) => {
    ...
    return (
        <>
            {!isLoggedIn && (
                <p className="mb-5 rounded-ui-xs border border-ui-gray-800 bg-ui-secondary p-3 text-ui-small text-ui-fg-muted">
            ...
            <SettingSection title={t('settings.system.data.sectionStorage')}>
                <div className="flex flex-col gap-3">
                    ...
                    <div className="rounded-ui-xs border border-ui-gray-800 bg-ui-secondary p-4">
                        <div className="mb-2 flex items-center justify-between text-ui-small">
                            <span className="text-ui-fg-muted">{t('settings.system.data.cacheUsageLabel')}</span>
                            <span className="font-ui-mono tabular-nums text-ui-fg">
            ...
            >
                <p className="text-ui-small text-ui-fg-muted">{t('settings.system.data.confirmIrreversible')}</p>
            </Modal>
        </>
    );
};
