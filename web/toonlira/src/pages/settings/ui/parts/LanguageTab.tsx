                ...
                <SettingRow label={t('settings.system.language.readingLangs')} desc={t('settings.system.language.readingLangsDesc')} block>
                    <ol className="flex flex-col gap-2 pt-1">
                        {readingLangs.map((lang, index) => {
                            ...
                            return (
                                <li key={lang} className="flex items-center gap-3 rounded-ui-xs border border-ui-separator bg-ui-secondary px-3 py-2">
                                    <span className="w-5 shrink-0 text-center text-ui-small font-ui-bold text-ui-fg-subtle">{index + 1}</span>
                                    <span className="min-w-0 flex-1 truncate text-ui-body font-ui-bold text-ui-fg">{t(labelKeyFor(lang))}</span>
                                    {isFallback && (
                                        <span className="shrink-0 rounded-ui-xs bg-ui-accent-10 px-1.5 py-0.5 text-ui-tiny font-ui-bold uppercase tracking-ui-label text-ui-accent-fg">
                                    ...
                                    <div className="flex shrink-0 items-center gap-1">
                                        <button
                                            ...
                                            className="ui-focus-ring inline-flex size-8 items-center justify-center rounded-ui-xs text-ui-fg-subtle hover:text-ui-fg disabled:cursor-not-allowed disabled:opacity-30"
                                        >
                                        ...
                                        <button
                                            ...
                                            className="ui-focus-ring inline-flex size-8 items-center justify-center rounded-ui-xs text-ui-fg-subtle hover:text-ui-danger disabled:cursor-not-allowed disabled:opacity-30"
                                        >
                    ...
                    {availableLangs.length > 0 && (
                        <div className="mt-3">
                            <p className="mb-2 text-ui-small text-ui-fg-subtle">{t('settings.system.language.availableLangs')}</p>
                            <div className="flex flex-wrap gap-2">
                                {availableLangs.map(opt => (
                                    <button
                                        ...
                                        className="ui-focus-ring inline-flex items-center gap-1.5 rounded-ui-xs border border-ui-separator px-2.5 py-1.5 text-ui-small font-ui-bold text-ui-fg hover:border-ui-accent-border hover:text-ui-accent-fg"
                                    >
