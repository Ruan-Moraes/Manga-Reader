...
export const SettingSection = ({ title, children }: { title: string; children: ReactNode }) => (
    <section className="mb-8">
        <h3 className="mb-1 text-[18px] font-ui-extrabold text-ui-fg">{title}</h3>
...
export const SettingRow = ({ label, desc, block, children }: SettingRowProps) => (
    <div className={cn('border-b border-ui-separator py-[18px]', block ? 'flex flex-col gap-3' : 'flex items-start justify-between gap-4')}>
        <div className={cn('min-w-0', !block && 'flex-1')}>
            <p className="text-ui-body font-ui-bold text-ui-fg">{label}</p>
            {desc && <p className="mt-0.5 max-w-[44ch] text-ui-small text-ui-fg-subtle">{desc}</p>}
        </div>
