...
const SystemSettings = () => {
    ...
    const syncMeta = {
        local: {
            ...
            icon: Info,
            indicator: 'bg-ui-fg-subtle',
            iconClassName: 'text-ui-fg-subtle',
        },
        idle: {
            label: t('settings.system.metaSyncing'),
            icon: LoaderCircle,
            indicator: 'bg-ui-accent',
            iconClassName: 'animate-spin text-ui-accent-fg',
        },
        syncing: {
            label: t('settings.system.metaSyncing'),
            icon: LoaderCircle,
        ...
        synced: {
            ...
            icon: CheckCircle2,
            indicator: 'bg-ui-accent',
            iconClassName: 'text-ui-accent-fg',
        },
        error: {
            label: t('settings.system.metaError'),
            icon: AlertCircle,
            indicator: 'bg-ui-danger',
            iconClassName: 'text-ui-danger',
        },
    }[state.syncStatus];
    ...
    const meta = (
        <span className="flex items-center gap-2">
            <span className={`inline-block size-2 shrink-0 rounded-ui-full ${syncMeta.indicator}`} aria-hidden="true" />
    ...
    return (
        <PageContainer asMain size="default" paddingY="md">
            ...
            {state.needsReload && (
                <div className="mb-6 flex flex-col gap-3 rounded-ui-xs border border-ui-accent-50 bg-ui-accent-10 p-4 sm:flex-row sm:items-center sm:justify-between">
                    <p className="flex items-center gap-2 text-ui-small text-ui-fg">
                        <RefreshCw className="size-4 shrink-0 text-ui-accent-fg" aria-hidden="true" />
            ...
            {state.syncStatus === 'error' && (
                <div className="mb-6 flex flex-col gap-3 rounded-ui-xs border border-ui-danger bg-ui-danger-15 p-4 sm:flex-row sm:items-center sm:justify-between">
                    <p className="flex items-center gap-2 text-ui-small text-ui-fg">
                        <AlertCircle className="size-4 shrink-0 text-ui-danger" aria-hidden="true" />
