...
const CategoryFilterPanel = ({ tags, selectedTags, onTagToggle, selectedStatus, onStatusChange, onClearAll, activeCount }: CategoryFilterPanelProps) => {
    ...
    return (
        <div className="rounded-ui-md border border-ui-border bg-ui-surface p-4 shadow-ui-elevated xl:p-5">
            <div className="mb-5 flex items-center justify-between gap-3 border-b border-ui-border pb-4">
                <div>
                    <p className="ui-label text-ui-fg-subtle">{t('filters.refine')}</p>
                    <h3 className="mt-1 text-ui-h4 font-ui-extrabold text-ui-fg">{t('filters.filtersButton')}</h3>
                </div>
                {activeCount > 0 && <span className="rounded-ui-full bg-ui-accent px-2.5 py-1 text-ui-tiny font-ui-bold text-ui-on-accent">{activeCount}</span>}
            </div>

            <div className="flex flex-col gap-5">
            <fieldset className="m-0 border-none p-0">
                <legend className="mb-2 text-ui-tiny font-ui-bold text-ui-fg">{t('filters.genres')}</legend>
                <SearchField
                    value={tagQuery}
                ...
                <div className="flex flex-col gap-2 max-h-64 overflow-y-auto">
                    {visibleTags.length === 0 ? (
                        <p className="py-2 text-ui-tiny text-ui-fg-subtle">{t('filters.noTagsFound')}</p>
                    ) : (
                        visibleTags.map(tag => (
                            <Checkbox
                                key={tag.value}
