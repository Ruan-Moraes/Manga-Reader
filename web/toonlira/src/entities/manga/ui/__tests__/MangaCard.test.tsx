...
describe('MangaCard', () => {
    ...
    it('renders author', () => {
        render(<MangaCard manga={manga} />);
        expect(screen.getByText('K. Miura')).toHaveClass('text-ui-on-overlay/80');
    });

    it('renders chapter number', () => {
        render(<MangaCard manga={manga} />);
        expect(screen.getByText(/cap\. 370/i).parentElement).toHaveClass('text-ui-on-overlay/80');
    });
    ...
    it('shows bookmark button when onToggleLibrary provided', () => {
        ...
        expect(screen.getByRole('button', { name: /salvar na biblioteca/i })).toHaveClass('!bg-ui-overlay-strong', '!text-ui-on-overlay');
    });

    it('shows remove label when inLibrary=true', () => {
        render(<MangaCard manga={manga} onToggleLibrary={vi.fn()} inLibrary />);
        expect(screen.getByRole('button', { name: /na biblioteca/i })).toHaveClass('!border-ui-accent', '!text-ui-accent');
    });
