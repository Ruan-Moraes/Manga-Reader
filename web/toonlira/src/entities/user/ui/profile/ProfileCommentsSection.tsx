...
import { type CommentSummary } from '../../model/user.types';
import { getUserComments } from '../../api/userService';
                ...
                <ul className="space-y-2">
                    {comments.map(c => (
                        <li key={c.id} className="p-3 border rounded-xs border-tertiary bg-secondary/30">
                            <Link to={`${WEB_BASE_URL}${ROUTES.TITLE_DETAIL(c.titleId)}`} className="text-xs text-ui-accent-fg hover:underline">
