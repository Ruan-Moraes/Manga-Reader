    ...
    return (
        <div className="flex gap-4">
            <button
                ...
                className="size-20 shrink-0 overflow-hidden rounded-ui-sm border border-ui-border bg-ui-surface-muted transition-colors hover:border-ui-accent-50 ui-focus-ring"
            >
                {user.photo ? (
                    <img src={user.photo} alt={t('modal.photoAlt', { name: user.name })} className="size-full object-cover" />
                ) : (
                    <span className="grid size-full place-items-center text-ui-h3 font-ui-extrabold text-ui-fg-subtle">{user.name.charAt(0).toUpperCase()}</span>
                )}
            </button>
            ...
            <div className="flex min-w-0 flex-1 flex-col gap-2">
                {(isModerator || isMember) && (
                    <ul className="flex flex-wrap gap-1.5">
                        {isModerator && (
                            <li className="inline-flex items-center gap-1 rounded-ui-full border border-ui-accent-50 bg-ui-accent-25 px-2 py-0.5 text-ui-tiny font-ui-extrabold uppercase tracking-wide text-ui-accent-fg">
                                <ShieldCheck className="size-3" aria-hidden="true" />
                                {t('modal.badges.moderator')}
                            </li>
                        )}
                        {isMember && (
                            <li className="inline-flex items-center gap-1 rounded-ui-full border border-ui-border bg-ui-surface-muted px-2 py-0.5 text-ui-tiny font-ui-extrabold uppercase tracking-wide text-ui-fg-muted">
                ...
                {(isModerator || isMember) && (
                    <div className="flex flex-col gap-0.5 text-ui-small text-ui-fg-subtle">
                ...
                <button
                    type="button"
                    onClick={handleGoToProfile}
                    className="inline-flex items-center gap-1.5 self-start text-ui-small font-ui-bold text-ui-accent-fg hover:underline ui-focus-ring"
                >
                    {t('modal.viewProfile')}
