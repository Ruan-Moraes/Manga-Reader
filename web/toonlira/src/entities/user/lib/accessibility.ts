...
const REDUCE_MOTION_CLASS = 'ui-reduce-motion';
const NO_ANIMATIONS_CLASS = 'ui-no-animations';
const HIGH_CONTRAST_CLASS = 'ui-high-contrast';

const FONT_CLASSES = ['ui-font-compact', 'ui-font-comfortable'] as const;
const DENSITY_CLASSES = ['ui-density-compact'] as const;
const THEME_CLASSES = ['ui-theme-dark', 'ui-theme-light', 'ui-theme-system'] as const;
...
export function applySystemPreferences(settings: UserSettings) {
    ...
    if (settings.appearance.fontSize === 'COMPACT') root.classList.add('ui-font-compact');
    if (settings.appearance.fontSize === 'COMFORTABLE') root.classList.add('ui-font-comfortable');

    root.classList.remove(...DENSITY_CLASSES);
    if (settings.appearance.density === 'COMPACT') root.classList.add('ui-density-compact');

    root.classList.remove(...THEME_CLASSES);
    root.classList.add(`ui-theme-${settings.appearance.theme.toLowerCase()}`);
