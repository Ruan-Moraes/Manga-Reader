...
const GroupMemberModal = ({ isOpen, user, closeModal }: GroupMemberModalProps) => {
        ...
        <Modal open={isOpen} onClose={closeModal} title={t('member.profileTitle')}>
            <section className="mx-auto w-full max-w-2xl">
                ...
                <p className="mt-1 text-xs text-tertiary">
                    {t('member.joinedAt', {
                        date: new Date(user.joinedAt).toLocaleDateString(i18n.language),
                    })}
                </p>

                <div className="mt-4">
                    <h5 className="text-sm font-bold">{t('member.participatesIn')}</h5>
                ...
                <div className="mt-4">
                    ...
                    <div className="flex flex-col gap-2 mt-2 max-h-56 overflow-y-auto pr-1">
                        {user.recentPosts.slice(0, 5).map(post => (
                            <a key={post.id} href={post.link} className="p-2 border rounded-xs border-tertiary hover:border-ui-accent-border transition-colors">
