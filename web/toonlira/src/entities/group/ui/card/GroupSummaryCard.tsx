...
const statusColorMap: Record<string, string> = {
    ...
    hiatus: 'bg-ui-warning',
    inactive: 'bg-ui-danger',
};

const statusTextMap: Record<string, string> = {
    active: 'text-ui-success',
    hiatus: 'text-ui-warning',
    inactive: 'text-ui-danger',
};
    ...
    return (
        <article className="flex flex-col gap-4 p-4 border rounded-xs border-tertiary bg-secondary/40 hover:border-quaternary-opacity-50 transition-colors">
            <div className="flex gap-3 items-center">
                <div className="shrink-0 w-14 h-14 rounded-full border border-ui-accent-border overflow-hidden">
                    {!avatarError ? (
                        <img
                            src={group.logo}
            ...
            <AppLink
                ...
                className="w-full px-3 py-2 text-xs text-center border rounded-xs border-tertiary hover:border-ui-accent-border transition-colors"
            />
        </article>
    );
};
