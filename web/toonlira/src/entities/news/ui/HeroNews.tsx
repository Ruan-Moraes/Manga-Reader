...
const HeroNews = ({ news }: { news: NewsSummary }) => {
    ...
    return (
        <article className="overflow-hidden rounded-ui-xs border border-ui-border bg-ui-surface">
            <Link to={ROUTES.NEWS_DETAIL(news.slug || news.id)} className="group grid lg:grid-cols-[1.35fr_1fr]">
                <div className="relative min-h-64 overflow-hidden bg-ui-secondary lg:min-h-[430px]">
                ...
                <div className="flex flex-col justify-center gap-5 p-6 sm:p-8 lg:p-10">
                    ...
                    <p className="text-ui-tiny font-ui-bold uppercase tracking-[0.1em] text-ui-accent-fg">{formatRelativeDate(news.publishedAt)}</p>
                    <h1 className="text-ui-h2 font-ui-extrabold leading-[1.05] tracking-mr text-ui-fg lg:text-[2.75rem]">{news.title}</h1>
                    <p className="text-ui-body leading-relaxed text-ui-fg-muted">{news.excerpt}</p>
                    <div className="flex flex-wrap items-center gap-4 text-ui-small text-ui-fg-subtle"><span className="inline-flex items-center gap-1.5"><Clock className="size-4" />{t('card.readMinutes', { count: news.readTime })}</span><span className="inline-flex items-center gap-1.5"><Eye className="size-4" />{news.views.toLocaleString(i18n.language)}</span></div>
                    <span className="inline-flex items-center gap-2 font-ui-bold text-ui-accent-fg">{t('page.readMore')}<ArrowRight className="size-4 transition group-hover:translate-x-1" /></span>
                </div>
            </Link>
        </article>
    );
