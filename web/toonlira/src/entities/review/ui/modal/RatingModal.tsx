        ...
        <div className="flex flex-col gap-4">
            ...
            <div className="flex items-center justify-between gap-3">
                <div className="min-w-0">
                    <p className="text-[11px] font-ui-bold uppercase tracking-ui-label text-ui-accent-fg">{t('modal.evaluateLabel')}</p>
                    {titleName && <p className="truncate text-[15px] font-ui-extrabold text-ui-fg">{titleName}</p>}
                </div>
                <div className="shrink-0 text-right">
                    <p className={cn('text-[28px] font-ui-extrabold leading-none tabular-nums', overall ? 'text-ui-accent-fg' : 'text-ui-fg-subtle')}>
                        {overall ? overall.toFixed(1) : '—'}
                    </p>
                    <p className="text-[11px] text-ui-fg-subtle">{t('modal.overallLabel')}</p>
                </div>
            </div>

            {/* Barra de progresso */}
            <div>
                <div className="mb-1 flex items-center justify-between text-[12px] text-ui-fg-subtle">
                    <span>{t('modal.progress', { rated, total: REVIEW_CRITERIA.length })}</span>
                    <span>{pct}%</span>
                </div>
                <div className="h-1.5 overflow-hidden rounded-full" style={{ background: 'var(--ui-gray-700)' }}>
                    <div className="h-full rounded-full transition-[width] duration-500" style={{ width: `${pct}%`, background: 'var(--ui-accent)' }} />
                </div>
            </div>
                ...
                {REVIEW_CRITERIA.map(c => {
                    ...
                    return (
                        <div
                            key={c.key}
                            className={cn(
                                'flex items-center justify-between gap-3 rounded-ui-xs border p-3 transition-colors duration-200',
                                val > 0 ? 'border-ui-accent-border bg-ui-accent-25' : 'border-ui-border bg-ui-surface-interactive',
                            )}
                        >
                            <div className="min-w-0">
                                <p className="text-[14px] font-ui-bold text-ui-fg">{name}</p>
                                <p className="text-[12px] text-ui-fg-subtle">{t(c.descKey)}</p>
                            </div>
            ...
            <div className="flex flex-col gap-2">
                <input
                    className="h-10 w-full rounded-ui-xs border border-ui-tertiary bg-ui-primary px-3 text-[14px] text-ui-fg outline-none transition-colors focus:border-ui-accent-border placeholder:text-ui-fg-subtle"
                ...
                />
                <textarea
                    className="min-h-[80px] w-full resize-none rounded-ui-xs border border-ui-tertiary bg-ui-primary px-3 py-2 text-[14px] text-ui-fg outline-none transition-colors focus:border-ui-accent-border placeholder:text-ui-fg-subtle"
                    rows={3}
                ...
                <div className="flex items-center justify-between">
                    <button
                        ...
                        className={cn(
                            'flex items-center gap-1.5 rounded-ui-xs px-3 py-1 text-[12px] font-ui-bold transition-colors',
                            spoiler
                                ? 'border border-[var(--ui-danger)] bg-ui-danger-15 text-[var(--ui-danger)]'
                                : 'border border-ui-tertiary bg-ui-secondary text-ui-fg-subtle hover:border-[var(--ui-danger)] hover:text-[var(--ui-danger)]',
                        )}
                    >
                        <span
                            className={cn('size-2 rounded-full transition-colors', spoiler ? 'bg-[var(--ui-danger)]' : 'bg-ui-tertiary')}
                            aria-hidden="true"
                        />
                        {t('modal.spoilerToggle')}
                    </button>
                    <span className="text-[12px] text-ui-fg-subtle">{comment.length}/600</span>
                </div>
            </div>
...
function ReviewSuccess({ overall, onDone }: { overall: number; onDone: () => void }) {
    ...
    return (
        <div className="flex flex-col items-center gap-4 py-4 text-center">
            ...
            <div>
                <h3 className="text-[20px] font-ui-extrabold text-ui-fg">{t('modal.successTitle')}</h3>
                <p className="mt-1 text-[14px] text-ui-fg-muted">{t('modal.successBody', { overall: overall.toFixed(1) })}</p>
            </div>
