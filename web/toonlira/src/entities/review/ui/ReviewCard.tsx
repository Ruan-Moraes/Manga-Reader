    ...
    return (
        <article onClick={onClick} className={cn(onClick && 'cursor-pointer')}>
            ...
            >
                                ...
                                {subjectTitle.onClick ? (
                                    <div className="flex gap-1.5 flex-col">
                                        <button
                                            ...
                                            className="min-w-0 truncate text-left text-[15px] font-ui-extrabold text-ui-fg transition-colors hover:text-ui-accent-fg"
                                        >
                                            {subjectTitle.label}
                                        </button>
                                        {genres && genres.length > 0 && (
                                            <div className="flex flex-wrap gap-2">
                                                {genres.map(genre => (
                                                    <span
                                                        key={genre}
                                                        className="rounded-ui-full border border-ui-chip-border bg-ui-chip px-2 py-0.5 text-ui-tiny font-ui-bold text-ui-fg-subtle"
                                                    >
                                                {genre}
                                            </span>
                                                ))}
                                    ...
                                    <h3 className="min-w-0 truncate text-[15px] font-ui-extrabold text-ui-fg">{subjectTitle.label}</h3>
                                )}
                                {!author && (
                                    <div className="flex shrink-0 flex-col items-end gap-1">
                                        <div className="flex items-center gap-1.5 text-ui-small text-ui-fg-subtle">
                                        ...
                                        <span className="hidden items-center gap-1.5 sm:flex">
                                            <span className="text-[15px] font-ui-extrabold tabular-nums text-ui-accent-fg">{rating.toFixed(1)}</span>
                ...
                {title && <h3 className="text-[15px] font-ui-extrabold text-ui-fg">{title}</h3>}
                ...
                {veiled ? (
                    <button
                        ...
                        className="flex w-full items-center justify-center gap-2 rounded-ui-xs border border-dashed border-ui-border-subtle bg-ui-surface-muted p-[18px] text-ui-small font-ui-bold text-ui-fg-subtle transition-all hover:border-ui-accent-50 hover:text-ui-accent-fg cursor-pointer"
                    >
                        <Eye className="size-4" aria-hidden="true" />
                        {t('card.showSpoiler')}
                    </button>
                ) : (
                    <div className={cn('text-ui-body leading-relaxed text-ui-fg-muted', collapsed && 'line-clamp-5')}>
                ...
                {isLong && !veiled && (
                    <button
                        ...
                        className="flex items-center gap-1 text-ui-small font-ui-bold text-ui-accent-fg hover:underline"
                    >
                ...
                {reviewScores && (
                    <div>
                        <button
                            ...
                            className="flex items-center gap-1.5 self-start text-ui-small font-ui-bold text-ui-fg-subtle transition-colors hover:text-ui-accent-fg"
                        >
                            <ChevronDown className={cn('size-3.5 transition-transform duration-200', breakOpen && 'rotate-180')} aria-hidden="true" />
                            {t('card.breakdown')}
                        </button>
                        {breakOpen && (
                            <div className="mt-1 flex flex-col gap-2.5 rounded-ui-xs border border-ui-border bg-ui-surface-muted p-3">
                                {REVIEW_CRITERIA.map(criterion => {
                                    ...
                                    return (
                                        ...
                                        >
                                            <span className="text-ui-small text-ui-fg-subtle">
                                                {isMobile && label.length > 8 ? `${label.slice(0, 8)}...` : label}
                                            </span>
                                            <Meter value={(value / 5) * 100} />
                                            <span className="text-right text-ui-small font-ui-bold text-ui-fg">{value.toFixed(1)}</span>
                                        </div>
                                    );
                                })}
                            </div>
                ...
                <div onClick={event => event.stopPropagation()}>
                    <ActionBar
                        ...
                        extra={
                            ...
                            (confirmingDelete ? (
                                <>
                                    <span className="text-ui-tiny text-ui-danger">{t('card.deleteConfirm')}</span>
                                    <button
                                        type="button"
                                        onClick={() => {
                                            onDelete?.();
                                        ...
                                        className="rounded-ui-xs px-2 py-1 text-ui-small font-ui-bold text-ui-danger hover:bg-ui-danger-15 cursor-pointer"
                                    >
                                        {t('card.deleteYes')}
                                    </button>
                                    <button
                                        ...
                                        className="rounded-ui-xs px-2 py-1 text-ui-small font-ui-bold text-ui-fg-subtle hover:bg-ui-secondary cursor-pointer"
                                    >
                                        {t('card.deleteNo')}
                                    </button>
                                </>
                                ...
                                <>
                                    {onEdit && (
                                        <button
                                            ...
                                            className="inline-flex items-center gap-1.5 rounded-ui-full border border-ui-chip-border bg-ui-chip px-[13px] py-1.5 text-[12.5px] font-ui-bold text-ui-fg-subtle transition-all hover:border-ui-accent-50 hover:text-ui-fg ui-focus-ring max-md:w-8 max-md:justify-center max-md:gap-0 max-md:px-0"
                                        >
                                    ...
                                    {onDelete && (
                                        <button
                                            ...
                                            className="inline-flex items-center gap-1.5 rounded-ui-full border border-ui-chip-border bg-ui-chip px-[13px] py-1.5 text-[12.5px] font-ui-bold text-ui-fg-subtle transition-all hover:border-ui-danger hover:text-ui-danger ui-focus-ring max-md:w-8 max-md:justify-center max-md:gap-0 max-md:px-0"
                                        >
