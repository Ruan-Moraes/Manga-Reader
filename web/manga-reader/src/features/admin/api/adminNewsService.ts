...
export const getAdminNews = async (page = 0, size = 20, search?: string, sort = 'publishedAt', direction = 'desc'): Promise<PageResponse<AdminNews>> => {
    const response = await api.get<ApiResponse<PageResponse<AdminNews>>>(API_URLS.ADMIN_NEWS, { params: { page, size, search, sort, direction } });
