...
const actionBtn = 'flex size-7 items-center justify-center rounded-mr-xs border border-mr-border bg-mr-surface text-mr-fg-muted transition-colors hover:border-mr-accent-50 hover:text-mr-accent';

/** Card de página: thumb + posição editável + badge do pipeline + ações. */
const ChapterPageCard = ({ page, selected, onToggleSelect, onRemove, onReplace, onRetry, onZoom, onMoveTo, totalPages }: ChapterPageCardProps) => {
    ...
    return (
        <div
            ...
            className={cn(
                ...
                selected ? 'border-mr-accent' : 'border-mr-border',
                isBroken && 'border-[rgba(255,120,79,0.55)]',
        ...
        >
            <div className="relative aspect-[3/4] w-full overflow-hidden bg-mr-gray-900">
                {isPending ? (
                    <div className="flex h-full flex-col items-center justify-center gap-2 text-mr-fg-muted">
                        ...
                        {/* Barra de progresso simulada — preparação p/ upload real (DT-44). */}
                    ...
                    <div className="flex h-full flex-col items-center justify-center gap-2 text-mr-danger">
                        ...
                        <button type="button" onClick={onRetry} className="inline-flex items-center gap-1 rounded-mr-xs border border-[rgba(255,120,79,0.4)] px-2 py-1 text-mr-tiny font-mr-bold hover:bg-mr-danger-15">
                ...
                <label className="absolute left-2 top-2 z-10 flex size-6 cursor-pointer items-center justify-center rounded-mr-xs bg-[rgba(0,0,0,0.55)]" onClick={e => e.stopPropagation()}>
                    <input
                        type="checkbox"
                        checked={selected}
                        onChange={onToggleSelect}
                        aria-label={t('dashboard.chapters.pages.selectAriaLabel', { order: page.order })}
                        className="size-3.5 cursor-pointer accent-[var(--color-mr-accent,#f97316)]"
                    />
                </label>
                ...
                <button
                    ...
                    className="absolute right-2 top-2 z-10 flex size-6 cursor-grab items-center justify-center rounded-mr-xs bg-[rgba(0,0,0,0.55)] text-mr-fg-muted hover:text-mr-accent"
                >
            ...
            <div className="flex items-center gap-1.5 px-2 py-2">
                ...
                <input
                    ...
                    className="h-7 w-12 rounded-mr-xs border border-mr-border bg-mr-primary px-1.5 text-center text-mr-tiny tabular-nums text-mr-fg focus:border-mr-accent focus:outline-none"
                />
            ...
            <div className="flex items-center justify-end gap-1 px-2 pb-2">
                ...
                <button type="button" onClick={onReplace} aria-label={t('dashboard.chapters.pages.replaceAriaLabel', { order: page.order })} className={actionBtn}>
                    <Replace size={13} />
                </button>
                <button
                    type="button"
                    ...
                    className={cn(actionBtn, 'border-[rgba(255,120,79,0.4)] text-mr-danger hover:bg-mr-danger-15 hover:text-mr-danger')}
                >
                    <Trash2 size={13} />
                </button>
            </div>
