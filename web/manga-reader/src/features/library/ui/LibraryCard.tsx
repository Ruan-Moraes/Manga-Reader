    ...
    return (
        <article className="flex gap-3 p-3 border rounded-xs border-tertiary bg-secondary/20 hover:bg-secondary/40 transition-colors">
            <AppLink link={`title/${manga.titleId}`}>
                <img src={manga.cover} alt={manga.name} className="object-cover w-20 h-28 rounded-xs flex-shrink-0" />
            </AppLink>
            <div className="flex flex-col flex-1 gap-2 min-w-0">
                <AppLink link={`title/${manga.titleId}`} text={manga.name} className="text-sm font-medium truncate" />
                ...
                {confirming ? (
                    <div className="flex items-center gap-2 text-xs">
                        <span className="text-quinary-default">{t('card.confirmPrompt')}</span>
                        <button
                            onClick={() => {
                        ...
                        <button onClick={() => setConfirming(false)} className="px-2 py-0.5 border rounded-xs border-tertiary hover:bg-tertiary/20">
                            {t('card.cancel')}
                        </button>
                    </div>
                ) : (
