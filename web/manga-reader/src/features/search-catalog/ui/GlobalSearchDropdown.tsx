    ...
    <div className="flex flex-col gap-2 p-2" aria-hidden="true">
        {Array.from({ length: 3 }).map((_, index) => (
            <div key={index} className="flex items-center gap-3">
                <Skeleton variant="rect" width={32} height={48} className="shrink-0 rounded-mr-sm" />
...
const GlobalSearchDropdown = ({
    ...
    return (
        <div
            className="absolute left-0 right-0 top-[calc(100%+8px)] z-50 max-h-[min(72vh,580px)] overflow-y-auto rounded-mr-lg border border-mr-separator bg-mr-secondary p-2 shadow-mr-elevated motion-safe:origin-top motion-safe:transition-opacity max-sm:fixed max-sm:left-3 max-sm:right-3 max-sm:top-16"
        >
            {!term && (
                <>
                    <div className="flex items-center gap-2 px-2 pb-2 pt-2">
                        <span className="flex size-7 items-center justify-center rounded-mr-sm bg-mr-accent-10 text-mr-accent-fg">
                            <Sparkles className="size-3.5" aria-hidden="true" />
                        </span>
                        <h2 className="text-xs font-mr-extrabold uppercase tracking-widest text-mr-accent-fg">{t('search.suggestionsTitle')}</h2>
                    </div>
                ...
                {term && queryReady && !searchLoading && !searchError && results.length > 0 && (
                    <>
                        {[...new Set(results.map(option => option.sectionType))].map(sectionType => {
                            ...
                            return (
                            <div key={sectionType} role="group" aria-label={t(`search.type.${sectionType}`)} className="border-b border-mr-separator py-1 last:border-b-0">
                                <div className="flex items-center gap-2 px-2 pb-1 pt-2">
                                    <Icon className="size-3.5 text-mr-accent-fg" aria-hidden="true" />
                                    <h2 className="text-xs font-mr-extrabold uppercase tracking-widest text-mr-accent-fg">
                        ...
                        {viewAll && (
                            <button
                                ...
                                className="group mt-2 flex w-full items-center justify-center gap-1.5 rounded-mr-sm border border-mr-accent-border bg-mr-accent-10 p-3 text-mr-small font-mr-bold text-mr-accent-fg motion-safe:transition-colors hover:bg-mr-accent-25"
                            >
            ...
            {term && term.length < 2 && <p className="p-4 text-center text-mr-small text-mr-fg-muted">{t('search.tooShort')}</p>}
            {term && term.length > 100 && <p className="p-4 text-center text-mr-small text-mr-fg-muted">{t('search.tooLong')}</p>}
            ...
            {term && queryReady && searchError && (
                <div className="flex flex-col items-center gap-3 p-5 text-center">
                    <AlertCircle className="size-5 text-mr-danger" aria-hidden="true" />
                    <p className="text-mr-small text-mr-fg-muted">{t('search.errorDescription')}</p>
            ...
            {term && queryReady && !searchLoading && !searchError && results.length === 0 && (
                <div className="flex flex-col items-center gap-2 p-5 text-center">
                    <Search className="size-5 text-mr-fg-muted" aria-hidden="true" />
                    <p className="text-mr-small text-mr-fg-muted">{t('search.noResults', { term })}</p>
                </div>
            )}
        </div>
    );
