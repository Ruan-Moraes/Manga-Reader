                ...
                {titles.map(title => (
                    <article key={title.id} className="min-w-0">
                        <AppLink link={ROUTES.TITLE_DETAIL(title.id)} className="block">
                            ...
                            <h3 className="mt-2 line-clamp-2 text-mr-small font-mr-bold text-mr-fg">{title.name}</h3>
                        </AppLink>
                        ...
                        {title.genres?.length > 0 && <p className="mt-1 truncate text-mr-tiny text-mr-fg-subtle">{title.genres.slice(0, 2).join(' · ')}</p>}
                    </article>
                ))}
            </div>
        </section>
