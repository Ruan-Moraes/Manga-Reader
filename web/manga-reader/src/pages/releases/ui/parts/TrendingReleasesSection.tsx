        ...
        <section className="mt-12" aria-label={t('releases.trending.title')}>
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {titles.slice(0, 6).map((title, index) => (
                    <article key={title.id} className="flex gap-3 rounded-mr-xs border border-mr-border bg-mr-surface p-3">
                        <span className="text-mr-h2 font-mr-extrabold text-mr-accent-fg" aria-label={t('releases.trending.position', { position: index + 1 })}>
                            {String(index + 1).padStart(2, '0')}
                        </span>
                        <div className="w-12 shrink-0"><MangaPoster cover={title.cover} alt="" size={48} radius="sm" /></div>
                        <div className="min-w-0">
                            <AppLink link={ROUTES.TITLE_DETAIL(title.id)} className="line-clamp-2 text-mr-small font-mr-bold text-mr-fg hover:text-mr-accent-fg">
                                {title.name}
                            </AppLink>
