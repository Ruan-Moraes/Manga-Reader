...
const NewReleases = () => {
    ...
    return (
        <PageContainer asMain size="default" paddingY="lg">
            <header className="mb-8 grid gap-5 border-b border-mr-border pb-8 lg:grid-cols-[1fr_auto] lg:items-end">
                <div className="max-w-3xl">
                    <p className="mr-label mb-2 text-mr-accent-fg">{t('releases.eyebrow')}</p>
                    <h1 className="text-mr-h1 font-mr-extrabold leading-tight tracking-mr text-mr-fg">{t('releases.title')}</h1>
                    <p className="mt-3 text-mr-body leading-relaxed text-mr-fg-muted">{t('releases.description')}</p>
                </div>
                <p className="text-mr-small text-mr-fg-subtle" aria-live="polite">
                    {t('releases.total', { count: total })}
                </p>
            </header>

            <section className="mb-8 rounded-mr-xs border border-mr-border bg-mr-surface p-4" aria-label={t('releases.filtersLabel')}>
                ...
                <div className="mt-3 border-t border-mr-border pt-3">
                    <Switch
                        ...
                        {groups.map(group => (
                            <section key={group.date} aria-labelledby={`release-day-${group.date}`}>
                                <div className="mb-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                                    <h2 id={`release-day-${group.date}`} className="text-mr-h3 font-mr-extrabold uppercase tracking-mr text-mr-fg">
