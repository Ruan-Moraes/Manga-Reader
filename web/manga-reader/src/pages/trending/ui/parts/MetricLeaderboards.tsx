    ...
    return (
        <section className="rounded-mr-sm border border-mr-border bg-mr-surface p-4 md:p-5">
            <header className="mb-4 flex items-start gap-3">
                <span className="inline-flex size-10 shrink-0 items-center justify-center rounded-mr-xs bg-mr-accent/10 text-mr-accent-fg"><Icon className="size-5" /></span>
                ...
                {items.map((item, index) => (
                    <li key={item.id}>
                        <button type="button" onClick={() => navigate(ROUTES.TITLE_DETAIL(item.id))}
                            ...
                            <div className="min-w-0 flex-1"><p className="truncate text-mr-small font-mr-bold text-mr-fg group-hover:text-mr-accent-fg">{item.name}</p><p className="text-mr-tiny text-mr-fg-muted">{item.metrics[metric].toLocaleString()}</p></div>
...
const MetricLeaderboards = ({ dashboard }: { dashboard: TrendingDashboard }) => {
    ...
    return (
        <section className="mt-12">
            <div className="mb-5"><p className="mr-label text-mr-accent-fg">{t('trending.metricsEyebrow')}</p><h2 className="mt-1 text-mr-h3 font-mr-bold text-mr-fg">{t('trending.metricsTitle')}</h2><p className="mt-2 max-w-2xl text-mr-small text-mr-fg-muted">{t('trending.metricsDescription')}</p></div>
