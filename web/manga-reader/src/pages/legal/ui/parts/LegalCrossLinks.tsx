...
export const LegalCrossLinks = ({ current }: { current: LegalPage }) => {
    ...
    return (
        <nav aria-label={t('crossLinks.ariaLabel')} className="mt-12 border-t border-mr-border-subtle pt-8">
            <p className="mr-label mb-4 text-mr-fg-subtle">{t('crossLinks.heading')}</p>
                ...
                {others.map(key => {
                    ...
                    return (
                        <Card key={key} variant="default" interactive onClick={() => navigate(meta.path)} className="flex min-w-0 items-center gap-3">
                            <div className="flex size-9 shrink-0 items-center justify-center rounded-mr-xs bg-mr-accent-25 text-mr-accent-fg">
                                <Icon className="size-4" />
                            </div>
                            <div className="min-w-0 flex-1">
                                <div className="text-mr-small font-mr-extrabold text-mr-fg truncate">{t(`crossLinks.pages.${key}.label`)}</div>
                                <div className="text-mr-tiny text-mr-fg-subtle truncate">{t(`crossLinks.pages.${key}.desc`)}</div>
                            </div>
