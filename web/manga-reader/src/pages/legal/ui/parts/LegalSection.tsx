    ...
    return (
        <section id={id} className={cn('scroll-mt-24 py-8', className)}>
            <h2 className="flex items-baseline gap-3 text-mr-h3 font-mr-extrabold tracking-mr text-mr-fg">
                <span className="font-mr-mono text-mr-small text-mr-accent tabular-nums">{String(num).padStart(2, '0')}</span>
                {title}
            </h2>
            ...
            <div className="prose-legal mt-4 flex flex-col gap-3 text-mr-body leading-relaxed text-mr-fg-muted">{children}</div>
        </section>
    );
};
