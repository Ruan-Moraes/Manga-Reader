...
const EventDetails = () => {
    ...
    if (!event) {
        return (
            <div className="events-page" style={{ textAlign: 'center', padding: '80px 20px' }}>
                <div style={{ color: '#999' }}>{t('notFound')}</div>
                <button type="button" onClick={back} style={{ marginTop: 16, padding: '10px 18px', background: 'var(--mr-accent)', color: '#161616', border: 'none', borderRadius: 2, fontWeight: 800, cursor: 'pointer' }}>
                    {t('backToEventsShort')}
                </button>
            </div>
        );
    ...
    const accent = event.accent || (event.type === 'special' ? 'var(--mr-accent)' : '#888');
        ...
        <main>
            <div style={{ position: 'relative', height: 'min(280px, 40vw)', minHeight: 200, background: event.cover, overflow: 'hidden' }}>
                ...
                <button
                    ...
                    style={{ position: 'absolute', top: 16, left: 16, zIndex: 2, width: 40, height: 40, borderRadius: 2, background: 'rgba(0,0,0,0.6)', border: '1px solid rgba(255,255,255,0.15)', color: '#fff', fontSize: 18, cursor: 'pointer', backdropFilter: 'blur(8px)' }}
                >
                    ‹
                </button>
            </div>
            ...
            <div className="events-page" style={{ marginTop: -60, position: 'relative', zIndex: 1 }}>
                <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 12, flexWrap: 'wrap' }}>
                    {isSpecial && (
                        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: '#000', color: accent, padding: '6px 12px', borderRadius: 2, fontSize: 11, fontWeight: 800, letterSpacing: '.12em', textTransform: 'uppercase', border: `1px solid ${accent}`, boxShadow: `0 0 12px ${accent}66` }}>
                ...
                <h1 style={{ fontSize: 'clamp(24px,5vw,36px)', fontWeight: 800, color: '#fff', margin: '0 0 12px', letterSpacing: '.0625rem', lineHeight: 1.15 }}>{event.name}</h1>
                <p style={{ fontSize: 15, color: '#bbb', lineHeight: 1.65, margin: '0 0 24px', letterSpacing: '.0625rem', maxWidth: 680 }}>{event.description}</p>
                ...
                {(status === 'active' || status === 'upcoming') && (
                    <div style={{ marginBottom: 24 }}>
                        <div style={{ fontSize: 11, fontWeight: 700, color: '#888', textTransform: 'uppercase', letterSpacing: '.12em', marginBottom: 8 }}>{status === 'active' ? t('ui.endsIn') : t('ui.startsIn')}</div>
                ...
                {event.rewards && event.rewards.length > 0 && (
                    <div style={{ marginBottom: 24 }}>
                        <h3 style={{ fontSize: 14, fontWeight: 800, color: '#fff', margin: '0 0 12px', letterSpacing: '.08em', textTransform: 'uppercase' }}>{t('ui.rewards')}</h3>
                ...
                <button
                    ...
                    style={{
                        width: '100%', maxWidth: 320, padding: '14px 20px', background: status === 'ended' ? '#2a2a2a' : accent, color: status === 'ended' ? '#666' : '#161616',
