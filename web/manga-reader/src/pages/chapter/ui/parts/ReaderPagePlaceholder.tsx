...
export const ReaderPagePlaceholder = ({ n, chapter, src }: ReaderPagePlaceholderProps) => {
    ...
    return (
        <div className="reader-page" data-rd-page={n} style={showImage ? undefined : { background: PAGE_GRADIENT }} role="img" aria-label={t('reader.pageAria', { page: n, chNum: chapter })}>
            ...
            {showImage ? (
                <img className="reader-page-img" src={imageSrc} alt="" loading="lazy" onError={() => setFailed(true)} />
            ) : src && failed ? (
                <div className="flex flex-col items-center gap-2.5 py-16 text-mr-fg-muted">
                    ...
                    <button
                        type="button"
                        onClick={retry}
                        className="inline-flex items-center gap-1.5 rounded-mr-xs border border-mr-border px-3 py-1.5 text-mr-tiny font-mr-bold text-mr-fg transition-colors hover:border-mr-accent-50 hover:text-mr-accent"
                    >
