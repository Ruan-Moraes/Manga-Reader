export enum QUERY_KEYS {
    // Titles
    TITLES = 'titles',
    TITLES_IN_THE_CAROUSEL = 'titlesCarousel',
    TITLES_ON_THE_RISE = 'titlesMostViewed',
    RANDOM_TITLES = 'randomTitles',
    UPDATED_TITLES = 'updatedTitles',
    TITLES_SEARCH = 'titlesSearch',

    // Chapters
    CHAPTERS = 'chapters',

    // Comments
    COMMENTS = 'comments',

    // Ratings / Reviews
    RATINGS_BY_TITLE = 'ratingsByTitle',
    RATING_AVERAGE = 'ratingAverage',
    RATING_DISTRIBUTION = 'ratingDistribution',
    USER_REVIEWS = 'userReviews',

    // Stores
    STORES_BY_TITLE = 'storesByTitle',

    // Library
    LIBRARY_COUNTS = 'libraryCounts',
    LIBRARY_IDS = 'libraryIds',
    LIBRARY_BY_USER = 'libraryByUser',

    // Groups
    GROUPS = 'groups',
    GROUPS_BY_TITLE = 'groupsByTitle',

    // Tags
    TAGS = 'tags',

    // News
    NEWS = 'news',

    // Events
    EVENTS = 'events',
    EVENT = 'event',
    EVENT_RELATED = 'eventRelated',

    // Forum
    FORUM_TOPICS = 'forumTopics',

    // Users
    USER_PROFILE = 'userProfile',
    USER_CONTENT_LOCALES = 'userContentLocales',

    // Social (DT-48)
    FOLLOWERS = 'followers',
    FOLLOWING = 'following',
    FOLLOWED_GROUPS = 'followedGroups',
    USER_SETTINGS = 'userSettings',

    // Admin
    ADMIN_USERS = 'adminUsers',
    ADMIN_USER_DETAIL = 'adminUserDetail',
    ADMIN_TITLES = 'adminTitles',
    ADMIN_TITLE_DETAIL = 'adminTitleDetail',
    ADMIN_CHAPTERS = 'adminChapters',
    ADMIN_CHAPTER_DETAIL = 'adminChapterDetail',
    ADMIN_CHAPTER_PAGES = 'adminChapterPages',
    ADMIN_CHAPTER_METRICS = 'adminChapterMetrics',
    ADMIN_CHAPTER_METRICS_OVERVIEW = 'adminChapterMetricsOverview',
    ADMIN_CHAPTER_READS_SERIES = 'adminChapterReadsSeries',
    READER_PAGES = 'readerPages',
    ADMIN_NEWS = 'adminNews',
    ADMIN_NEWS_DETAIL = 'adminNewsDetail',
    ADMIN_EVENTS = 'adminEvents',
    ADMIN_EVENT_DETAIL = 'adminEventDetail',
    ADMIN_GROUPS = 'adminGroups',
    ADMIN_GROUP_DETAIL = 'adminGroupDetail',
    ADMIN_DASHBOARD_METRICS = 'adminDashboardMetrics',
    ADMIN_DASHBOARD_CONTENT_METRICS = 'adminDashboardContentMetrics',
    ADMIN_PAYMENTS = 'adminPayments',
    ADMIN_PAYMENT_DETAIL = 'adminPaymentDetail',
    ADMIN_FINANCIAL_SUMMARY = 'adminFinancialSummary',
    ADMIN_REVENUE_SERIES = 'adminRevenueSeries',
    ADMIN_SUBSCRIPTIONS = 'adminSubscriptions',
    ADMIN_SUBSCRIPTION_SUMMARY = 'adminSubscriptionSummary',
    ADMIN_SUBSCRIPTION_GROWTH = 'adminSubscriptionGrowth',
    ADMIN_TAGS = 'adminTags',
    ADMIN_AUTHORS = 'adminAuthors',
    ADMIN_PUBLISHERS = 'adminPublishers',
    ADMIN_PLANS = 'adminPlans',
    ADMIN_SUBSCRIPTION_LOGS = 'adminSubscriptionLogs',

    // Domain Labels
    DOMAIN_LABELS = 'domainLabels',
}
