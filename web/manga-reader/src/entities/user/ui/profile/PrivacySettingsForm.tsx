...
import { type PrivacySettings, type VisibilitySetting } from '../../model/user.types';
import { updatePrivacySettings } from '../../api/userService';
...
const PrivacySettingsForm = ({ privacySettings, onUpdate }: Props) => {
    ...
    const handleSave = async () => {
        if (historyVis === 'DO_NOT_TRACK' && privacySettings.viewHistoryVisibility !== 'DO_NOT_TRACK') {
            const confirmed = window.confirm(t('profile.privacy.doNotTrackConfirm'));
            if (!confirmed) return;
        }
    ...
    return (
        <div className="px-4 py-3 space-y-6">
            <div>
                <h3 className="text-sm font-semibold mb-3">{t('profile.privacy.commentVisibility')}</h3>
                <div className="space-y-2">
                    {OPTIONS.filter(o => o.value !== 'DO_NOT_TRACK').map(opt => (
                        <label key={opt.value} className="flex items-center gap-2 cursor-pointer">
                            ...
                            />
                            <div>
                                <span className="text-sm">{t(`profile.privacy.${opt.labelKey}`)}</span>
                                <p className="text-[10px] text-tertiary">{t(`profile.privacy.${opt.descriptionKey}`)}</p>
                            </div>
                        </label>
                    ))}
                </div>
            ...
            <div>
                <h3 className="text-sm font-semibold mb-3">{t('profile.privacy.historyVisibility')}</h3>
                <div className="space-y-2">
                    {OPTIONS.map(opt => (
                        <label key={opt.value} className="flex items-center gap-2 cursor-pointer">
                            <input
