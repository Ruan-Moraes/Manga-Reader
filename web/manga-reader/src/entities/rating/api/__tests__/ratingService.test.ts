...
describe('ratingService', () => {
    ...
    describe('getRatingsByTitleId - campos avançados (DT-45)', () => {
        it('deve mapear reviewTitle, spoiler, top, upvotes, downvotes e myVote', async () => {
            ...
            server.use(http.get(`*${API_URLS.RATINGS}/title/title-1`, () => HttpResponse.json({ data: buildPageResponse([rating]), success: true })));
    ...
    describe('castReviewVote', () => {
        it('deve enviar value e retornar contadores + myVote', async () => {
            server.use(
                http.post(`*${API_URLS.RATINGS}/rating-1/vote`, () => HttpResponse.json({ data: { upvotes: 8, downvotes: 1, myVote: 'up' }, success: true })),
            );
    ...
    describe('removeReviewVote', () => {
        it('deve remover voto e retornar contadores com myVote nulo', async () => {
            server.use(
                http.delete(`*${API_URLS.RATINGS}/rating-1/vote`, () => HttpResponse.json({ data: { upvotes: 7, downvotes: 1, myVote: null }, success: true })),
            );
