...
describe('useReviewVote', () => {
    it('registra voto e reconcilia contadores com a resposta do servidor', async () => {
        server.use(
            http.get(`*${API_URLS.RATINGS}/title/t1`, () => HttpResponse.json(listBody([review()]))),
            http.post(`*${API_URLS.RATINGS}/r1/vote`, () => HttpResponse.json({ data: { upvotes: 6, downvotes: 1, myVote: 'up' }, success: true })),
        );
    ...
    it('toggle do mesmo lado remove o voto (DELETE)', async () => {
        server.use(
            http.get(`*${API_URLS.RATINGS}/title/t1`, () => HttpResponse.json(listBody([review({ upvotes: 6, myVote: 'up' })]))),
            http.delete(`*${API_URLS.RATINGS}/r1/vote`, () => HttpResponse.json({ data: { upvotes: 5, downvotes: 1, myVote: null }, success: true })),
        );
    ...
    it('faz rollback do cache quando a requisição falha', async () => {
        server.use(
            http.get(`*${API_URLS.RATINGS}/title/t1`, () => HttpResponse.json(listBody([review()]))),
            http.post(`*${API_URLS.RATINGS}/r1/vote`, () => HttpResponse.json(null, { status: 500 })),
        );
