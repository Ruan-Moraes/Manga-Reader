...
const NewsCard = ({ news }: { news: NewsSummary }) => {
    ...
    return (
        <article className="group overflow-hidden rounded-mr-xs border border-mr-border bg-mr-surface transition duration-mr-default hover:-translate-y-1 hover:border-mr-accent-50">
            <Link to={ROUTES.NEWS_DETAIL(news.slug || news.id)} className="flex h-full flex-col focus-visible:outline focus-visible:outline-2 focus-visible:outline-mr-focus-ring">
                ...
                <div className="flex flex-1 flex-col gap-3 p-4">
                    <p className="text-mr-tiny font-mr-bold uppercase tracking-[0.08em] text-mr-accent-fg">{formatRelativeDate(news.publishedAt)}</p>
                    ...
                    <div className="flex items-center justify-between border-t border-mr-border-subtle pt-3 text-mr-tiny text-mr-fg-subtle">
                        ...
                        <ArrowUpRight className="size-4 text-mr-accent-fg" aria-hidden="true" />
                    </div>
                </div>
            </Link>
        </article>
