package com.toonlira.application.review.usecase;

import org.springframework.stereotype.Service;

import com.toonlira.application.manga.port.TitleRatingAggregateReadPort;
import com.toonlira.application.review.port.ReviewRepositoryPort.ReviewDistribution;

import lombok.RequiredArgsConstructor;

/**
 * Retorna a distribuição de avaliações de um título por faixa de estrela (1–5).
 * <p>
 * Lê do agregado consolidado ({@code title_rating_aggregate}, mantido pelo
 * serviço {@code rating-aggregator}) — sem agregação ao vivo durante a
 * renderização. Título sem agregado ⇒ distribuição vazia.
 */
@Service
@RequiredArgsConstructor
public class GetReviewDistributionUseCase {
    private final TitleRatingAggregateReadPort aggregateReadPort;

    public ReviewDistribution execute(String titleId) {
        return aggregateReadPort.findByTitleId(titleId)
                .map(v -> new ReviewDistribution(v.star1(), v.star2(), v.star3(), v.star4(), v.star5()))
                .orElseGet(ReviewDistribution::empty);
    }
}
