package com.toonlira.application.contact.usecase;
...
public class PublishWorkContactUseCase {
    ...
    @Value("${app.mail.admin-email:admin@toonlira.com}")
    ...
    private void sendAdminNotification(PublishWorkInput input) {
        ...
        emailPort.sendHtml(
                adminEmail,
                "Toonlira — Nova Solicitação de Publicação: " + input.workTitle(),
                htmlBody
        );
    }
    ...
    private void sendAuthorConfirmation(PublishWorkInput input) {
        ...
        emailPort.sendHtml(
                input.email(),
                "Toonlira — Recebemos sua solicitação de publicação",
                htmlBody
        );
    }
