...
class SearchNewsUseCaseTest {
    ...
    void deveRetornarNoticiasCorrespondentes() {
        ...
        List<NewsItem> items = List.of(
                NewsItem.builder().title(com.toonlira.shared.domain.i18n.LocalizedString.ofDefault("One Piece capítulo 1120")).category(NewsCategory.LANCAMENTOS).build(),
                NewsItem.builder().title(com.toonlira.shared.domain.i18n.LocalizedString.ofDefault("One Piece: novo arco confirmado")).category(NewsCategory.PRINCIPAIS).build()
        );
