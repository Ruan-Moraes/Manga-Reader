...
- Os documentos em `reviews` são **escritos** pelos use cases do `api/apps/core`; este
...
# Build do JAR
./mvnw -pl apps/jobs/rating-aggregator -am package -DskipTests
java -jar apps/jobs/rating-aggregator/target/rating-aggregator-0.0.1-SNAPSHOT.jar
...
```bash
./mvnw -pl apps/jobs/rating-aggregator -am test -Dtest=RecalculateTitleRatingUseCaseTest,TitleRatingReconciliationJobTest,AdminReconcileControllerTest  # unit (sem Docker)
./mvnw -pl apps/jobs/rating-aggregator -am test                                                                                                         # + integração Mongo (Testcontainers, requer Docker)
```
