# Orphan Cleaner — `api/apps/jobs/orphan-cleaner`
...
- A infra é **subida pela API principal** (`api/apps/core`, via `spring-boot-docker-compose`);
  este serviço apenas se conecta. Por isso `spring.docker.compose.enabled=false`.
- Contadores e refs são **mantidos** pelos use cases do `api/apps/core`; este serviço só os
  corrige/limpa.
...
```bash
# Local (dev)
./mvnw -pl apps/jobs/orphan-cleaner spring-boot:run

# Build do JAR
./mvnw -pl apps/jobs/orphan-cleaner -am package -DskipTests
java -jar apps/jobs/orphan-cleaner/target/orphan-cleaner-0.0.1-SNAPSHOT.jar
...
```bash
./mvnw -pl apps/jobs/orphan-cleaner -am test -Dtest.excludedGroups=testcontainers   # suíte leve (unit do Job), sem Docker
./mvnw -pl apps/jobs/orphan-cleaner -am test                                         # + integração Postgres/Mongo (Testcontainers, requer Docker)
```
