...
- Espelha o serviço irmão [`counter-reconciler`](../counter-reconciler) (mesmo padrão
...
- Os documentos em `reviews` são **escritos** pelos use cases do `api/server`; este
...
Pré-requisito: MongoDB + RabbitMQ no ar (suba a infra pelo `api/server`:
...
cd ../server && docker compose -f docker-compose.prod.yml up -d rating-aggregator
```
