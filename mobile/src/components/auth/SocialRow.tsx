...
function SocialButton({ icon, label }: { icon: 'google' | 'apple'; label: string }) {
  return (
    <TouchableOpacity
      activeOpacity={0.8}
      style={{
        flex: 1, height: MR.controlHeight, flexDirection: 'row',
        alignItems: 'center', justifyContent: 'center', gap: 9,
        backgroundColor: MR.surface, borderWidth: 1, borderColor: MR.tertiary,
        borderRadius: MR.radius,
      }}
    >
      <MRIcon name={icon} size={20} color={MR.text} />
      <Text style={{ fontFamily: FONTS.bold, fontSize: 14, color: MR.text, letterSpacing: MR.ls }}>
        {label}
      </Text>
    </TouchableOpacity>
  );
