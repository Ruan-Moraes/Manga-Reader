...
export function StrengthMeter({ value }: { value: string }) {
    const { radii, spacing, tokens, typography } = useTheme();
    ...
    return (
        <View style={{ marginTop: -spacing.sm, marginBottom: spacing.md }}>
            <View style={{ flexDirection: 'row', gap: spacing.xs }}>
                {[1, 2, 3, 4].map(i => (
                    <View key={i} style={{ flex: 1, height: 4, borderRadius: radii.pill, backgroundColor: i <= score ? tone : tokens.inputBorder }} />
                ))}
            </View>
            <Text
                style={{
