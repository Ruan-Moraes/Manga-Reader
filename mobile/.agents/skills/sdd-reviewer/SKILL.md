...
2. Confirmar que o gate estava aberto antes do planejamento e que todas as dependências declaradas estavam `implemented`.
3. Comparar cada `AC-*` com comportamento implementado e evidência, sem confiar apenas nos checkboxes.
4. Procurar requisitos ausentes, comportamento extra, regressões, violações FSD/i18n/tema e testes frágeis.
5. Confirmar que cada arquivo implementado está mapeado ao `AC-*` ou baseline correspondente e rodar `pnpm check`.
6. Criar ou atualizar `review.md` com findings priorizados e verdict `approved` ou `changes-requested`.
...
- Implementação iniciada com gate bloqueado exige `changes-requested`, mesmo que os ACs pareçam atendidos.
