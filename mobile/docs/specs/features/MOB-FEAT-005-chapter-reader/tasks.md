...
| -------- | ------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| AC-001   | TASK-001, TASK-002, TASK-005, TASK-009, TASK-011 | Unitários do mapper com páginas inválidas, repetidas e fora de ordem; contrato GET; integração da lista ordenada e estado localizado de capítulo vazio                         |
| AC-002   | TASK-003, TASK-005, TASK-009, TASK-011           | Matriz unitária de `VERTICAL`/`PAGED`/`DOUBLE` × `LTR`/`RTL`/`WEBTOON`, interação de troca de modo e caso ímpar de DOUBLE sem repetição                                        |
| AC-003   | TASK-004, TASK-005, TASK-011                     | Testes de limites de fit/saturação/gap/fundo/preload, integração com o único store de settings e remount provando restauração sem estado paralelo                              |
...
| AC-006   | TASK-006, TASK-007, TASK-009, TASK-011           | Integração de retomada válida, escolha entre capítulo solicitado/recente, progresso inválido, logout, expiração, resposta tardia e troca entre duas identidades                |
...
| AC-008   | TASK-005, TASK-008, TASK-009, TASK-011           | Falha/retry isolado de imagem e rede, posição preservada, pending observável, retry da versão mais recente, resposta obsoleta ignorada e nenhuma duplicação/regressão          |
| AC-009   | TASK-004, TASK-005, TASK-009, TASK-010, TASK-011 | Testes de roles/labels/hints/disabled/touch target, redução de movimento e escala de fonte; teste de rotação preservando página lógica e matriz manual iOS/Android documentada |
...
- `src/app/reader/[titleId]/[number].tsx` e eventual `_layout.tsx` fino, se necessário para full-screen
...
- `docs/specs/coverage.json`, `docs/specs/registry.md` e baseline/reconciliação diretamente afetados
...
- Composição: `src/pages/reader/**`, `src/app/reader/**` e registro no RootNavigator; integração guest prova somente GET público e zero chamadas `/users/me`.
...
- [x] TASK-011 — C02: virtualizar páginas com dimensões conhecidas, restauração lógica e espaço acessível de retry; testes de 10/67/200 páginas, fit e retomada distante.

TASK-010 permanece aberta: medir memória/frames, rotação, retorno, VoiceOver/TalkBack, imagens pequenas e fonte ampliada em Android/iOS release.
