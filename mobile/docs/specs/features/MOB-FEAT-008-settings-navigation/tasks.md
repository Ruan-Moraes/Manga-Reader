...
- [x] TASK-009 — Adicionar `src/app/settings/_layout.tsx`, `index.tsx` e as sete cascas finas de subrota, todas exportando pages pelas APIs públicas. O Stack interno deve usar títulos localizados, respeitar redução de movimento, possuir índice como origem segura para cold deep link e preservar histórico/back; registrar somente o grupo `settings` no navegador raiz, sem lógica de capacidade nos arquivos Expo Router.
...
7. `src/app/settings`: arquivos Expo Router finos, layout Stack e exports das pages; nenhuma regra de domínio.
...
- `src/app/settings/_layout.tsx`, `src/app/settings/index.tsx`, `src/app/settings/{appearance,locale,content-languages,reader,privacy,data,about}.tsx`
...
- Parar a task afetada se exigir import horizontal entre features, deep import externo, regra de settings em `shared`, lógica de sync no widget/page ou regra de capacidade dentro de arquivos `src/app/`.
