...
function RegisterScreen({
  layout = 'mascote', radius = 2, heading = 'Criar conta',
  showSocial = true, reviewState = 'interativo', onNavigate = () => {},
}) {
  ...
  const set = (k, v, fn) => { fn(v); if (errors[k]) setErrors((p) => { const n = { ...p }; delete n[k]; return n; }); };
  ...
  const submit = (ev) => {
    ...
    setLoading(true);
    timer.current = setTimeout(() => { setLoading(false); setSuccess(true); }, 800);
  };
    ...
    return (
      <div style={window.authScreenStyle({ justifyContent: 'center', alignItems: 'center', textAlign: 'center' })}>
        <div style={{ position: 'relative', marginBottom: 22 }}>
          ...
          <img src="assets/surpresa.png?v=2" alt="" width={148} height={148}
        ...
        <button type="button" onClick={() => onNavigate('login')} style={{
          ...
          border: 'none', borderRadius: radius, fontFamily: MR.font, fontSize: 14, fontWeight: 800,
  ...
  return (
    <div style={window.authScreenStyle()}>
      <window.AuthHeader
        layout={layout} radius={radius} mascot="surpresa.png?v=2"
        eyebrow="Junte-se à comunidade" title={heading}
      ...
      <form onSubmit={submit} noValidate style={{ display: 'flex', flexDirection: 'column' }}>
        <window.Field
          ...
          value={email} onChange={(v) => set('email', v, setEmail)}
          placeholder="voce@email.com" error={e.email}
        />
        <window.Field
          label="Nome de exibição" icon="user" radius={radius} autoComplete="nickname"
          value={name} onChange={(v) => set('name', v, setName)}
          placeholder="Como a comunidade vai te ver" error={e.name}
        />
        <window.Field
          label="Senha" type={showPw ? 'text' : 'password'} icon="lock" radius={radius}
          autoComplete="new-password"
          value={pw} onChange={(v) => set('pw', v, setPw)}
          placeholder="Crie uma senha forte" error={e.pw}
          trailing={
        ...
        <window.Field
          label="Confirmar senha" type={showPw ? 'text' : 'password'} icon="lock" radius={radius}
          autoComplete="new-password"
          value={pw2} onChange={(v) => set('pw2', v, setPw2)}
          placeholder="Repita a senha" error={e.pw2}
        />
        ...
        {e.terms && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, margin: '-8px 0 12px', fontSize: 11, color: MR.danger, letterSpacing: MR.ls }}>
            <MRIcon name="alert" size={13} /><span>{e.terms}</span>
          </div>
        )}
        ...
        <div style={{ height: 6 }} />
        <window.PrimaryButton type="submit" loading={showLoading} radius={radius}>Criar conta</window.PrimaryButton>
