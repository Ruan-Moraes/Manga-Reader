...
function Wordmark({ size = 18 }) {
  return (
    <span style={{
      ...
      letterSpacing: '1.4px', color: MR.text, lineHeight: 1, whiteSpace: 'nowrap',
    }}>
...
function LogoMark({ box = 40, radius = 2 }) {
  return (
    <div style={{
      width: box, height: box, borderRadius: radius, overflow: 'hidden',
      flexShrink: 0, background: '#000',
      boxShadow: '0 0 0 1px rgba(221,218,42,0.25)',
    }}>
      <img src="assets/logo.png" alt="" width={box} height={box}
...
function AuthHeader({ layout = 'mascote', eyebrow, title, sub, mascot, radius = 2, titleSize }) {
  ...
  const Title = (
    <h1 style={{
      margin: 0, fontSize: titleSize || (layout === 'minimal' ? 30 : 26), fontWeight: 800,
      lineHeight: 1.15, letterSpacing: MR.ls, color: MR.text, textWrap: 'balance',
    }}>{title}</h1>
  );
  const Sub = sub ? (
    <p style={{
      margin: '10px 0 0', fontSize: 13, lineHeight: 1.5, color: MR.subtle,
      letterSpacing: MR.ls, maxWidth: 320,
    }}>{sub}</p>
  ) : null;
  ...
  return (
    <div style={{ textAlign: 'center', marginBottom: 26 }}>
      <div style={{
        display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 9, marginBottom: 18,
      }}>
      ...
      {mascot && (
        <div style={{ position: 'relative', display: 'inline-block', marginBottom: 14 }}>
          ...
          <img src={'assets/' + mascot} alt="" width={108} height={108}
...
// ── Campo ──────────────────────────────────────────────────
function Field({
  label, type = 'text', value, onChange, placeholder, icon, error, hint,
  autoComplete, inputMode, rightSlot, radius = 2, trailing, onBlur,
}) {
    ...
    <label style={{ display: 'block', marginBottom: 16 }}>
      <div style={{
        ...
        <span style={{
          ...
          color: error ? MR.danger : 'var(--accent)', whiteSpace: 'nowrap',
        }}>{label}</span>
        {rightSlot}
      </div>
      <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
        {icon && (
          <span style={{
            position: 'absolute', left: 14, display: 'flex', pointerEvents: 'none',
            color: focused ? 'var(--accent)' : MR.tertiary, transition: 'color .15s',
          }}>
        ...
        <input
          ...
          style={{
            ...
            background: MR.inputBg, color: MR.text,
            border: `1px solid ${border}`, borderRadius: radius,
            fontFamily: MR.font, fontSize: 15, letterSpacing: MR.ls, outline: 'none',
      ...
      {error && typeof error === 'string' && error.trim() ? (
        <div style={{
          ...
          fontSize: 11, color: MR.danger, letterSpacing: MR.ls,
        }}>
      ...
      <div style={{ display: 'flex', gap: 5 }}>
        {[1, 2, 3, 4].map((i) => (
          <span key={i} style={{
            ...
            background: i <= score ? tone : MR.inputBorder, transition: 'background .2s',
          }} />
        ))}
      </div>
      <output aria-live="polite" style={{
        display: 'block', marginTop: 6, fontSize: 11, letterSpacing: MR.ls,
        color: score ? tone : MR.tertiary, minHeight: 14,
      }}>
...
function AuthCheckbox({ checked, onChange, children, error, radius = 2 }) {
  return (
    <button type="button" onClick={onChange} style={{
      ...
      border: 'none', padding: '2px 0', margin: '0 0 14px', cursor: 'pointer',
      color: error ? MR.danger : MR.muted, fontFamily: MR.font, fontSize: 13,
      letterSpacing: MR.ls, textAlign: 'left', lineHeight: 1.45, width: '100%',
    }}>
      <span style={{
        width: 20, height: 20, flexShrink: 0, marginTop: 1, borderRadius: Math.min(radius, 4),
...
function Spinner({ color }) {
  return (
    <span style={{
      ...
      display: 'inline-block', animation: 'mr-spin .7s linear infinite',
    }} />
  );
}
...
function PrimaryButton({ children, onClick, type = 'button', loading, disabled, radius = 2 }) {
  ...
  return (
    <button type={type} onClick={onClick} disabled={off} style={{
      height: 54, width: '100%', border: 'none', borderRadius: radius,
      background: off ? MR.inputBorder : 'var(--accent)',
      color: off ? '#666' : MR.bg,
      fontFamily: MR.font, fontSize: 14, fontWeight: 800,
      letterSpacing: '0.12em', textTransform: 'uppercase',
      cursor: off ? 'not-allowed' : 'pointer',
      display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10,
      whiteSpace: 'nowrap', transition: 'background .2s',
    }}>
      {loading ? (<><Spinner color="#666" /> Aguarde…</>) : children}
    </button>
  );
}
...
function GhostButton({ children, onClick, icon, radius = 2, disabled, accentShadow = true }) {
  return (
    <button type="button" onClick={onClick} disabled={disabled} style={{
      height: 54, width: '100%', background: 'transparent',
      color: disabled ? MR.tertiary : MR.text,
      border: `1px solid ${MR.tertiary}`, borderRadius: radius,
      boxShadow: accentShadow && !disabled ? '-0.25rem 0.25rem 0 0 var(--accent-25)' : 'none',
      fontFamily: MR.font, fontSize: 14, fontWeight: 700, letterSpacing: MR.ls,
  ...
  return (
    <button type="button" onClick={onPress}
      ...
      style={{
        ...
        gap: 9, background: MR.surface, color: MR.text,
        border: `1px solid ${MR.tertiary}`, borderRadius: radius,
        fontFamily: MR.font, fontSize: 14, fontWeight: 700, letterSpacing: MR.ls,
  ...
  return (
    <>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, margin: '22px 0 18px' }}>
        ...
        <span style={{
          fontSize: 11, color: MR.tertiary, letterSpacing: '0.08em',
...
function AuthFooter({ prompt, action, onAction }) {
  return (
    <div style={{
      marginTop: 24, paddingTop: 20, borderTop: `1px solid ${MR.inputBorder}`,
      textAlign: 'center', fontSize: 13, color: MR.subtle, letterSpacing: MR.ls,
    }}>
      {prompt}{' '}
...
function authScreenStyle(extra = {}) {
  return {
    ...
    background: MR.bg, color: MR.text, fontFamily: MR.font, letterSpacing: MR.ls,
    padding: '58px 24px 36px', display: 'flex', flexDirection: 'column', ...extra,
  };
}

Object.assign(window, {
  MR_TOKENS: MR, DEMO_CREDS: DEMO, Wordmark, LogoMark, AuthHeader, Field,
  passwordStrength, StrengthMeter, AuthCheckbox, Spinner, PrimaryButton,
  GhostButton, SocialButton, SocialRow, AuthFooter, authScreenStyle,
});
